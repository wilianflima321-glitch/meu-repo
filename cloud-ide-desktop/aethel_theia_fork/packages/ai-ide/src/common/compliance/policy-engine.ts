...
export class PolicyEngine {
  ...
  private readonly onPolicyViolationEmitter = new Emitter<PolicyViolation>();
  readonly onPolicyViolation: Event<PolicyViolation> = this.onPolicyViolationEmitter.event;

  private readonly onApprovalRequiredEmitter = new Emitter<ApprovalRequest>();
  readonly onApprovalRequired: Event<ApprovalRequest> = this.onApprovalRequiredEmitter.event;
   ...
   * Request approval
   */
  async requestApproval(context: PolicyContext, violations: PolicyViolation[]): Promise<ApprovalRequest> {
    const estimatedCost = this.estimateCost(context);
    const estimatedRisk = this.estimateRisk(context, violations);
   ...
   */
  private async loadRulesFromConfig(): Promise<void> {
    const strictMode = this.configService.get<boolean>('policy.strictMode', true);
    
    // Load code domain rules
    const codeRules = this.configService.get<PolicyRule[]>('policy.rules.code', [
      {
  ...
  private async evaluateRule(rule: PolicyRule, context: PolicyContext): Promise<{ passed: boolean }> {
  ...
  private checkPlanLimits(context: PolicyContext): { allowed: boolean; requiresApproval: boolean; violations: PolicyViolation[] } {
  ...
  private estimateCost(context: PolicyContext): number {
  ...
  private estimateRisk(context: PolicyContext, violations: PolicyViolation[]): 'low' | 'medium' | 'high' {
   ...
   */
  private async loadPlanLimitsFromConfig(): Promise<void> {
   ...
   */
  async checkPolicy(action: string, context: any): Promise<PolicyEvaluation> {
