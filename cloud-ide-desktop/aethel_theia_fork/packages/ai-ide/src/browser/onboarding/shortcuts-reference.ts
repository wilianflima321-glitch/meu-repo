...
export const AI_IDE_SHORTCUTS: Shortcut[] = [
    // AI Panel
    {
        id: 'ai.panel.toggle',
...
export function getShortcutsByCategory(category: string): Shortcut[] {
    return AI_IDE_SHORTCUTS.filter(s => s.category === category);
}

export function getShortcutKeys(shortcutId: string): string {
    const shortcut = AI_IDE_SHORTCUTS.find(s => s.id === shortcutId);
    if (!shortcut) return '';
...
export function getAllCategories(): string[] {
    const categories = new Set(AI_IDE_SHORTCUTS.map(s => s.category));
