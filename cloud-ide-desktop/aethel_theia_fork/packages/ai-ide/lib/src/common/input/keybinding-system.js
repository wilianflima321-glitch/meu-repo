...
exports.KeybindingSystem = exports.SpecialKey = exports.KeyModifier = void 0;
...
(function (KeyModifier) {
    ...
    KeyModifier["CtrlCmd"] = "ctrlcmd"; // Ctrl on Win/Linux, Cmd on Mac
...
(function (SpecialKey) {
    ...
    // Special characters
    SpecialKey["Semicolon"] = "semicolon";
    SpecialKey["Equal"] = "equal";
    SpecialKey["Comma"] = "comma";
    SpecialKey["Minus"] = "minus";
    SpecialKey["Period"] = "period";
    SpecialKey["Slash"] = "slash";
    SpecialKey["Backquote"] = "backquote";
    SpecialKey["BracketLeft"] = "bracketleft";
    SpecialKey["Backslash"] = "backslash";
    SpecialKey["BracketRight"] = "bracketright";
    SpecialKey["Quote"] = "quote";
})(SpecialKey || (exports.SpecialKey = SpecialKey = {}));
// ==================== Main Keybinding System ====================
let KeybindingSystem = class KeybindingSystem {
    constructor() {
        // Keybinding registry
        this.keybindings = new Map();
        this.keybindingsByKey = new Map();
        this.keybindingsByCommand = new Map();
        // Platform
        this.platform = 'windows';
        // Chord handling
        this.currentChord = [];
        this.chordTimeout = null;
        this.CHORD_TIMEOUT = 2000;
        // Context evaluation
        ...
        // Events
        this.onKeybindingChangedEmitter = new Emitter();
        this.onKeybindingChanged = this.onKeybindingChangedEmitter.event;
        this.onKeyPressedEmitter = new Emitter();
        this.onKeyPressed = this.onKeyPressedEmitter.event;
        this.onChordStartedEmitter = new Emitter();
        this.onChordStarted = this.onChordStartedEmitter.event;
        this.onChordCancelledEmitter = new Emitter();
        this.onChordCancelled = this.onChordCancelledEmitter.event;
        // Command executor
    ...
    registerKeybinding(keybinding) {
        ...
        if (typeof keybinding.key === 'string') {
            keybinding.key = this.parseKeyString(keybinding.key);
        }
     ...
     */
    unregisterKeybinding(keybindingId) {
    ...
    handleKeyDown(event) {
        ...
        activeMatches.sort((a, b) => {
            ...
            if (a.source === 'user' && b.source !== 'user')
                return -1;
            if (b.source === 'user' && a.source !== 'user')
                return 1;
            // Then weight
     ...
     */
    handleChordKeyDown(combination, event) {
        ...
        // Build current chord
        const currentChordObj = { parts: [...this.currentChord] };
     ...
     */
    findMatchingKeybindings(combination) {
        const chord = { parts: [combination] };
        return this.getKeybindingsForKey(this.serializeKeyChord(chord));
    }
    /**
     * Execute keybinding
     */
    async executeKeybinding(keybinding) {
     ...
     */
    setContextEvaluator(evaluator) {
        this.contextEvaluator = evaluator;
    }
    /**
     * Evaluate context expression
     */
    evaluateContext(expression) {
        if (!expression)
            return true;
        if (!this.contextEvaluator)
            return true;
     ...
     */
    findConflictingPairs(keybindings) {
        const pairs = [];
     ...
     * Check if when clauses could overlap
     */
    whenClausesCouldOverlap(when1, when2) {
     ...
     */
    extractContextKeys(expression) {
        const regex = /[a-zA-Z_][a-zA-Z0-9_.-]*/g;
        const matches = expression.match(regex) || [];
        return matches.filter(m => !['true', 'false', 'and', 'or', 'not', 'in', 'notin'].includes(m.toLowerCase()));
    }
    // ==================== Presets ====================
    /**
    ...
    importPreset(json) {
        ...
        for (const kb of preset.keybindings) {
            if (typeof kb.key === 'string') {
                kb.key = this.parseKeyString(kb.key);
            }
        }
     ...
     * Set user keybinding
     */
    setUserKeybinding(keybinding) {
        const userKb = {
            ...keybinding,
            source: 'user',
            key: typeof keybinding.key === 'string'
                ? this.parseKeyString(keybinding.key)
                : keybinding.key
        };
     ...
     */
    removeUserKeybinding(keybindingId) {
     ...
     * Disable keybinding
     */
    disableKeybinding(keybindingId) {
        this.disabledKeybindings.add(keybindingId);
    }
    /**
     * Enable keybinding
     */
    enableKeybinding(keybindingId) {
...
exports.KeybindingSystem = KeybindingSystem;
exports.KeybindingSystem = KeybindingSystem = __decorate([
    (0, inversify_1.injectable)(),
    __metadata("design:paramtypes", [])
], KeybindingSystem);
// ==================== Export ====================
exports.default = KeybindingSystem;
