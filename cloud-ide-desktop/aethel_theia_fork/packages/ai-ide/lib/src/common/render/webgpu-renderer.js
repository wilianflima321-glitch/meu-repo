...
var WebGPURenderer_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.BuiltinShaders = exports.WebGPURenderer = void 0;
 ...
 */
let WebGPURenderer = WebGPURenderer_1 = class WebGPURenderer {
    constructor() {
        this.device = null;
        this.adapter = null;
        this.context = null;
        ...
        this.frameIndex = 0;
        // Events
        this.onInitializedEmitter = new common_1.Emitter();
        ...
        this.onDeviceLostEmitter = new common_1.Emitter();
        ...
        this.onDeviceLost = this.onDeviceLostEmitter.event;
    }
    // ========================================================================
    // INITIALIZATION
    // ========================================================================
    ...
    async initialize(canvas) {
        if (this.initialized)
            return true;
        if (!WebGPURenderer_1.isSupported()) {
    ...
    createBuffer(id, data, usage) {
        ...
        new Uint8Array(mappedRange).set(new Uint8Array(ArrayBuffer.isView(data) ? data.buffer : data));
        buffer.unmap();
        const handle = {
            id,
            buffer,
     ...
     * Update buffer data
     */
    updateBuffer(id, data, offset = 0) {
        if (!this.device)
            return;
        const handle = this.buffers.get(id);
        if (!handle)
            return;
        this.device.queue.writeBuffer(handle.buffer, offset, ArrayBuffer.isView(data) ? data.buffer : data, ArrayBuffer.isView(data) ? data.byteOffset : 0, data.byteLength);
        ...
        for (const u of usage) {
            switch (u) {
                ...
                case 'indirect':
                    ...
                    break;
                case 'query-resolve':
                    flags |= GPUBufferUsage.QUERY_RESOLVE;
                    break;
            }
        }
        return flags;
     ...
     * Create a texture
     */
    createTexture(id, width, height, format = 'rgba8unorm', options = {}) {
        if (!this.device)
            return null;
     ...
     */
    async createTextureFromImage(id, source, options = {}) {
        if (!this.device)
            return null;
        ...
        this.device.queue.copyExternalImageToTexture({ source: imageBitmap }, { texture: handle.texture }, { width, height });
    ...
    updateTexture(id, data, options = {}) {
        ...
        this.device.queue.writeTexture({ texture: handle.texture, mipLevel, origin: { x, y } }, ArrayBuffer.isView(data) ? data.buffer : data, { bytesPerRow: width * 4, rowsPerImage: height }, { width, height });
    ...
    createRenderPipeline(config) {
        ...
        const buffers = config.vertexBuffers.map(layout => ({
        ...
        const targets = config.colorTargets.map(target => ({
        ...
        if (config.depthStencil) {
            descriptor.depthStencil = {
                format: config.depthStencil.format,
     ...
     */
    createComputePipeline(id, shaderCode, entryPoint = 'main') {
        if (!this.device)
            return null;
     ...
     * Get or create shader module
     */
    getOrCreateShaderModule(id, code) {
     ...
     * Begin a render pass
     */
    beginRenderPass(encoder, config) {
        const colorAttachments = config.colorAttachments.map(att => ({
    ...
    draw(pass, command) {
        ...
        if (command.indexBuffer) {
            ...
            if (handle) {
                ...
                pass.drawIndexed(command.indexCount || 0, command.instanceCount || 1, command.firstIndex || 0, 0, command.firstInstance || 0);
                this.currentStats.triangles += Math.floor((command.indexCount || 0) / 3);
            }
        }
        else {
            pass.draw(command.vertexCount || 0, command.instanceCount || 1, command.firstVertex || 0, command.firstInstance || 0);
    ...
    dispatch(encoder, command) {
        ...
        pass.dispatchWorkgroups(command.workgroupCountX, command.workgroupCountY || 1, command.workgroupCountZ || 1);
        pass.end();
    }
    // ========================================================================
    // MESH HELPERS
     ...
     */
    createSphereGeometry(radius = 0.5, segments = 32, rings = 16) {
        const positions = [];
        const normals = [];
        const uvs = [];
        const indices = [];
     ...
     * Create bind group
     */
    createBindGroup(pipeline, groupIndex, entries) {
        if (!this.device)
            return null;
...
exports.WebGPURenderer = WebGPURenderer = WebGPURenderer_1 = __decorate([
