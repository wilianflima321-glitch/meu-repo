import { CommandService, ILogger, QuickInputService, QuickPickInput, QuickPickItem } from '@theia/core';
...
type KernelPick = QuickPickItem & {
    kernel: NotebookKernel;
};
type GroupedKernelsPick = QuickPickItem & {
    kernels: NotebookKernel[];
    source: string;
};
type SourcePick = QuickPickItem & {
    action: SourceCommand;
};
type InstallExtensionPick = QuickPickItem & {
    extensionIds: string[];
};
type KernelSourceQuickPickItem = QuickPickItem & {
    command: NotebookCommand;
    documentation?: string;
};
type KernelQuickPickItem = (QuickPickItem & {
    autoRun?: boolean;
...
export declare class NotebookKernelQuickPickService {
    ...
    protected displaySelectAnotherQuickPick(editor: NotebookModel, kernelListEmpty: boolean): Promise<boolean>;
    protected isUri(value: string): boolean;
    protected calculateKernelSources(editor: NotebookModel): Promise<QuickPickInput<KernelQuickPickItem>[]>;
    protected selectOneKernel(notebook: NotebookModel, source: string, kernels: NotebookKernel[]): Promise<void>;
    protected executeCommand<T>(notebook: NotebookModel, command: NotebookCommand): Promise<T | undefined | void>;
}
export {};
