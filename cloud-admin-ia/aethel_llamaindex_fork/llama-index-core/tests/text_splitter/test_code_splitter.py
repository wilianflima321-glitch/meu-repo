...
@pytest.mark.skipif(SHOULD_SKIP, reason="tree_sitter not installed")
def test_json_code_splitter() -> None:
    """Test case for code splitting using json."""
    if "CI" in os.environ:
        return

    code_splitter = CodeSplitter(
        language="json", chunk_lines=4, chunk_lines_overlap=1, max_chars=50
    )
...
def test_markdown_code_splitter() -> None:
    """Test case for code splitting using markdown."""
    if "CI" in os.environ:
        return

    code_splitter = CodeSplitter(
        language="markdown", chunk_lines=2, chunk_lines_overlap=1, max_chars=50
    )
...
"""
    ...
    assert chunks[1].startswith("This is a paragraph.")


@pytest.mark.skipif(SHOULD_SKIP, reason="tree_sitter not installed")
def test_bash_code_splitter() -> None:
    """Test case for code splitting using bash."""
    if "CI" in os.environ:
        return

    code_splitter = CodeSplitter(
        language="bash", chunk_lines=2, chunk_lines_overlap=1, max_chars=50
    )
...
"""
    ...
    chunks = code_splitter.split_text(text)
    assert chunks[0].startswith("#!/bin/bash")
    assert chunks[1].startswith('echo "Hello, World!"')
