...
export interface DesktopCapabilityManifest {
  ...
  runtimeTemplatesPolicy: 'quarantined-not-ship-path' | 'absorb-into-studio-local'
...
export const V29_DESKTOP_CAPABILITY_MANIFEST: DesktopCapabilityManifest = {
  ...
  capabilities: [
    ...
    },
    {
      ...
      os: 'all',
      state: 'needs-review',
      evidenceRefs: ['apps/studio-local/src-tauri/src/sidecars.rs', 'runtime-templates/QUARANTINED.md'],
      nextAction:
        'Electron runtime-templates are quarantined (not ship). Connect Tauri sidecars to signed download/update/recovery flow.',
    },
    {
      id: 'native-renderer',
...
export const V29_DESKTOP_BRIDGE_COMMAND_CONTRACT: V29DesktopBridgeCommandContract = {
  ...
  commands: [
    ...
    },
    {
      ...
      purpose: 'Spawn a portable-pty shell session on the user machine (Studio Local), sandboxed to project root.',
      state: 'available',
      evidenceRefs: ['apps/studio-local/src-tauri/src/desktop_commands.rs', 'apps/studio-local/src/desktop-bridge/createDesktopAdapter.ts'],
    },
    {
      command: 'terminal_write',
      purpose: 'Write stdin to a live portable-pty session.',
      state: 'available',
      evidenceRefs: ['apps/studio-local/src-tauri/src/desktop_commands.rs', 'apps/studio-local/src/desktop-bridge/createDesktopAdapter.ts'],
    },
    {
      command: 'terminal_close',
      purpose: 'Close a portable-pty session and drop the native session record.',
      state: 'available',
      evidenceRefs: ['apps/studio-local/src-tauri/src/desktop_commands.rs', 'apps/studio-local/src/desktop-bridge/createDesktopAdapter.ts'],
    },
    {
