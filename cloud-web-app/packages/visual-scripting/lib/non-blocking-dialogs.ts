...
/**
 * Package-local copy of the pure-DOM confirm dialog. Originally lived in
 * `web/lib/ui/non-blocking-dialogs.ts` and was imported by relative path
 * (`../../web/lib/ui/...`) from this package — a Golden Rule 1 violation
 * (packages must not reach into `web/`, even without the `@/` alias). The
 * implementation itself has zero Next.js/Node dependency (plain
 * `document`/DOM APIs), so it is copied here verbatim rather than injected,
 * keeping `@aethel/visual-scripting` runnable with zero `web/` imports.
 */
