export type NodeCategory =
  ...
  | 'ability';   // Gameplay Ability System (OMNI-PLAN GAS — see lib/gas/)
...
export const NODE_CATALOG: NodeDefinition[] = [
  ...
  // ── ABILITY NODES (OMNI-PLAN GAS) ────────────────────────────────────────
  // Executed by `lib/visual-script/runtime-core/executors.ts` against the
  // Data-Oriented GAS engine in `lib/gas/` — see that module's doc comment
  // for how it relates to the legacy `lib/gameplay-ability-system.ts`.

  {
    type: 'ability_apply_effect',
    category: 'ability',
    label: 'Apply Gameplay Effect',
    description: 'Applies a registered GameplayEffect (buff/DOT/instant) to a target — e.g. "Fireball" -> Apply Effect (Burn)',
    inputs: [
      { id: 'exec', label: '', type: 'exec' },
      { id: 'target', label: 'Target', type: 'object' },
      { id: 'source', label: 'Source', type: 'object' },
      { id: 'effectId', label: 'Effect Id', type: 'string', default: 'Burn' },
    ],
    outputs: [
      { id: 'exec', label: '', type: 'exec' },
      { id: 'applied', label: 'Applied', type: 'boolean' },
    ],
    color: 'var(--aethel-error)',
  },
  {
    type: 'ability_check_tag',
    category: 'ability',
    label: 'Check Gameplay Tag',
    description: 'Checks whether a target currently holds a GameplayTag (hierarchical — "State.Debuff" matches "State.Debuff.Stun")',
    inputs: [
      { id: 'target', label: 'Target', type: 'object' },
      { id: 'tag', label: 'Tag', type: 'string', default: 'State.Debuff.Stun' },
    ],
    outputs: [{ id: 'hasTag', label: 'Has Tag', type: 'boolean' }],
    color: 'var(--aethel-error)',
  },
  {
    type: 'ability_wait_delay',
    category: 'ability',
    label: 'Wait Delay',
    description: 'Pauses this branch of the Ability Graph for a fixed duration before continuing',
    inputs: [
      { id: 'exec', label: '', type: 'exec' },
      { id: 'duration', label: 'Duration (s)', type: 'number', default: 1 },
    ],
