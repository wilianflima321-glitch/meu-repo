...
export type PaletteMode = 'commands' | 'files' | 'symbols' | 'lines' | 'all'

/**
 * A Visual Scripting node the Universal Search (FASE 3.5) can surface and
 * insert directly — host apps inject the live catalog (see
 * `@aethel/visual-scripting/visual-node-catalog`) via `scriptingNodes` on
 * `CommandPaletteProvider`, keeping this package free of a hard dependency
 * on the visual-scripting package (Golden Rule 1).
 */
export interface ScriptingNodeItem {
  type: string
  label: string
  category: string
  description?: string
...
export interface CommandPaletteProviderProps {
  ...
  onOpenSettings?: () => void
  files?: FileItem[]
  /** Visual Scripting node catalog for the Universal Search (FASE 3.5) — see `ScriptingNodeItem`. */
  scriptingNodes?: ScriptingNodeItem[]
  /** Called when the user picks a scripting node result — host inserts it into the active graph. */
