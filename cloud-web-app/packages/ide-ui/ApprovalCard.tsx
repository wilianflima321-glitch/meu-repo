                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-2 flex-1">
                      <input
                        type="checkbox"
                        id={`change-${change.filePath.replace(/[^a-z0-9]/gi, '-')}`}
                        checked={isSelected}
                        onChange={() => toggleChangeSelection(change.filePath)}
                        aria-label={`Select ${change.filePath} for bulk action`}
                        className="mt-1 accent-[var(--aethel-primary)]"
                      />
                  ...
                  {/* Diff Preview — truncated to prevent layout explosion on large files */}
                  {viewingChange === change.filePath && (
                    <div className="mt-3 rounded-lg border border-[var(--aethel-border-secondary)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_60%,transparent)] p-2 font-mono text-[10px]">
                      <div className="text-[var(--aethel-error-light)] whitespace-pre-wrap break-all">
                        {`- ${change.oldContent.slice(0, 200)}${change.oldContent.length > 200 ? '…' : ''}`}
                      </div>
                      <div className="mt-1 text-[var(--aethel-success-light)] whitespace-pre-wrap break-all">
                        {`+ ${change.newContent.slice(0, 200)}${change.newContent.length > 200 ? '…' : ''}`}
                      </div>
                    </div>
                  )}
                </div>
