...
export function WorkbenchPreviewPane({
  ...
  const handleApplyProposal = useCallback(async () => {
    ...
    try {
      const { runGovernedChangeApply } = await import(
        '../../../web/lib/ai/governed-change-apply-client'
      )
      ...
      if (!governed.ok) {
        toast.warning(governed.copy.title, governed.copy.detail)
        return
      }
      ...
      toast.warning(
        'Apply failed',
        error instanceof Error ? error.message : 'Governed apply failed.'
      )
    }
