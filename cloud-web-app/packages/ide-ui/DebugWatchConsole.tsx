...
export function ConsoleOutput({ messages, onClear, filter }: ConsoleOutputProps) {
  ...
  // Debug sessions can run for hours — cap the DOM to the most recent window
  // so a long-lived session never drags the whole IDE down with it.
  ...
  const filteredMessages = useMemo(() => {
    ...
    return filtered.length > MAX_RENDERED_MESSAGES
      ? filtered.slice(filtered.length - MAX_RENDERED_MESSAGES)
      : filtered
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-2 py-1 border-b border-[var(--aethel-border-secondary)]">
        ...
        <span className="text-xs text-[var(--aethel-text-tertiary)] ml-auto font-mono tabular-nums">
