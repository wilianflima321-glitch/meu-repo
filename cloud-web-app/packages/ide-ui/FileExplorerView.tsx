...
import { VirtualList } from '../../web/components/ui/VirtualList'
...
interface FileExplorerViewProps {
  ...
  /** Focus 1B — host-disk (Tauri) vs workspace disk (web) */
  treeAuthority?: 'disk' | 'host-disk' | null
  hostWatchActive?: boolean
  /** Studio Local — open real OS folder via picker/prompt */
  onOpenHostFolder?: () => void | Promise<void>
  hostFolderBusy?: boolean
...
export function FileExplorerView({
  ...
  const [treeContainerRef, treeContainerHeight] = useMeasuredHeight<HTMLDivElement>()

  // Flattening + virtualizing is what lets this explorer stay instant on
  // repos with tens of thousands of files — only rows in (or near) the
  // viewport ever touch the DOM, regardless of how many folders are expanded.
  const flattenedEntries = useMemo(
    () => flattenVisibleFiles(filteredFiles, expandedFolders),
  ...
  return (
    <div className={`h-full flex flex-col ${className}`}>
      <ExplorerHeader
        ...
        onRefresh={onRefresh}
        onOpenHostFolder={onOpenHostFolder}
        hostFolderBusy={hostFolderBusy}
        showOpenHostFolder={Boolean(onOpenHostFolder) || treeAuthority === 'host-disk'}
      />
      {lastSyncAt || treeAuthority ? (
        <ExplorerSyncStatus
      ...
      <div ref={treeContainerRef} className="flex-1 overflow-hidden py-1">
        {effectiveError ? <ExplorerError message={effectiveError} /> : null}
        {effectiveLoading && !effectiveError ? <ExplorerLoading /> : null}
        {flattenedEntries.length > 0 && treeContainerHeight > 0 ? (
          <VirtualList
            items={flattenedEntries.map(({ node, depth }) => ({ id: node.id, node, depth }))}
            itemHeight={FILE_TREE_ROW_HEIGHT}
            height={treeContainerHeight}
            overscan={8}
            selectedId={selectedFile}
            onSelect={(id) => {
              const entry = flattenedEntries.find(({ node }) => node.id === id)
              if (entry) onSelect(entry.node)
            }}
        ...
        <ExplorerEmptyState
          isVisible={flattenedEntries.length === 0 && !effectiveLoading && !effectiveError}
  ...
  return (
    <div className="density-header flex items-center justify-between px-2 border-b border-[var(--aethel-border-primary)]">
      ...
      <div className="flex items-center gap-1">
        {showOpenHostFolder && onOpenHostFolder ? (
          <IconButton
            label="Open host project folder"
            title="Open host folder"
            icon="folder-opened"
            className={iconButtonClass}
            onClick={onOpenHostFolder}
            disabled={hostFolderBusy}
          />
        ) : null}
...
function IconButton({
  ...
  label: string
  title: string
  icon: 'search' | 'new-file' | 'new-folder' | 'refresh' | 'folder-opened'
  className: string
  active?: boolean
  disabled?: boolean
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className={`${className} ${active ? 'text-[var(--aethel-info-light)]' : 'text-[var(--aethel-text-tertiary)]'} disabled:opacity-40`}
      title={title}
    >
      <Codicon name={icon} />
    </button>
...
function ExplorerSyncStatus({
  ...
  const authorityLabel =
    treeAuthority === 'host-disk'
      ? hostWatchActive
        ? 'Host disk · live watch'
        : 'Host disk'
      : treeAuthority === 'disk'
        ? 'Workspace disk'
        : null
  ...
  return (
    <div
      className="flex items-center justify-between gap-2 px-2 py-1 border-b border-[var(--aethel-border-primary)] text-[10px] text-[var(--aethel-text-tertiary)]"
      aria-live="polite"
      data-tree-authority={treeAuthority ?? 'unknown'}
      data-host-watch={hostWatchActive ? '1' : '0'}
    >
      <span>{lastSyncAt ? `Last synced: ${new Date(lastSyncAt).toLocaleTimeString()}` : 'Sync pending'}</span>
      {authorityLabel ? (
        <span className="shrink-0 text-[var(--aethel-info-light)]" title="Focus 1B tree authority">
          {authorityLabel}
        </span>
      ) : null}
    </div>
