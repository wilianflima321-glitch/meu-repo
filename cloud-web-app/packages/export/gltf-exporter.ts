...
/**
 * A single glTF morph target (blend shape): sparse per-vertex position/normal
 * deltas applied on top of the base mesh, weighted by
 * `KHR default weight / mesh.weights[i]`. This is the real, interop-standard
 * mechanism glTF/Unreal/Unity use for facial blendshapes — there is no
 * separate "blendshape" format, morph targets ARE blendshapes.
 */
export interface ExportMorphTarget {
  name: string;
  /** Same vertex count/order as the base mesh's `positions`. */
...
export interface ExportMesh {
  ...
  /** Facial/shape-key blend shapes (glTF morph targets). */
  morphTargets?: ExportMorphTarget[];
  /** Nanite meshlet metadata as extras (coarse LOD hint only — NOT a portable
   * cluster format; no interop engine currently imports real Nanite cluster
   * data via glTF/USD, so this stays informational metadata rather than a
   * claim of lossless Nanite round-tripping). */
  meshletCount?: number;
  lodError?: number;
}
...
export interface GLTFDocument {
  ...
  meshes: Array<{
    name: string;
    /** Default morph target weights, one per entry in each primitive's `targets`. */
    weights?: number[];
    primitives: Array<{
      attributes: Record<string, number>;
      indices?: number;
      material?: number;
      /** One entry per morph target: `{ POSITION: accessor, NORMAL?: accessor }`. */
...
export function exportSceneToGLTF(scene: ExportScene): { gltf: GLTFDocument; bin: ArrayBuffer } {
  ...
  const gltfMeshes = scene.meshes.map(mesh => {
    ...
    // Morph targets (facial/shape-key blend shapes) — one glTF target entry
    // per ExportMorphTarget, each contributing its own POSITION/NORMAL delta
    // accessors. `mesh.weights` carries the default (rest-pose) weights, and
    // `extras.targetNames` preserves human-readable names (glTF has no native
    // per-target name field — `targetNames` in extras is the ecosystem
    // convention Unreal/Unity/Blender importers already read).
    const targets = (mesh.morphTargets ?? []).map((target) => {
      const targetAttrs: Record<string, number> = {
        POSITION: addFloat32Accessor(target.positionDeltas, 'VEC3'),
      };
      if (target.normalDeltas) {
        targetAttrs.NORMAL = addFloat32Accessor(target.normalDeltas, 'VEC3');
      }
      return targetAttrs;
    });
    ...
    return {
      name: mesh.name,
      weights: targets.length > 0 ? targets.map(() => 0) : undefined,
      primitives: [{
        attributes: attrs,
        indices: indicesAcc,
        ...
        extras: {
          ...
          ...(targets.length > 0 ? { targetNames: (mesh.morphTargets ?? []).map((t) => t.name) } : {}),
        },
      }],
    };
  });
