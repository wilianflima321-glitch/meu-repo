...
/** J-ACC-04 — persisted TaskEvidenceLedger receipt */
export interface AIChatLedgerArtifact {
  kind: 'ledger'
  taskId: string
  projectId: string
  mission: string
  ...
  visualEvidenceHeld?: boolean
}

export type AIChatEvidenceArtifact =
  | AIChatTraceArtifact
...
export function buildLedgerEvidenceArtifact(input: unknown): AIChatLedgerArtifact | null {
  if (!isRecord(input)) return null
  ...
  if (!taskId || !projectId || !mission || !updatedAt) return null

  const events = Array.isArray(input.events)
    ? input.events
        .filter(isRecord)
        .map((event) => ({
          kind: asString(event.kind) ?? 'artifact',
          title: asString(event.title) ?? 'Evidence',
          summary: asString(event.summary) ?? '',
          refs: Array.isArray(event.refs)
            ? (event.refs.map(asString).filter(Boolean).slice(0, 8) as string[])
            : [],
        }))
        .slice(0, 24)
    : []

  const fusionTx = events
    .find((e) => e.kind === 'rollback')
    ?.refs.find((r) => r.startsWith('tx:'))
  const visualHeld = events.some((e) => e.kind === 'screenshot' && /HELD/i.test(e.title))
  ...
  return {
    ...
    events,
    fusionTransactionId: fusionTx?.replace(/^tx:/, ''),
    visualEvidenceHeld: visualHeld,
  }
}
