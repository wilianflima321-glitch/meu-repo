...
export default function CommandPalette({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
      ...
      {/* Palette */}
      <div className="relative w-[600px] max-h-[60vh] bg-[var(--aethel-surface-secondary)] rounded-lg shadow-2xl border border-[var(--aethel-border-primary)] overflow-hidden">
        {/* Input */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--aethel-border-primary)]">
          <Search className="w-5 h-5 text-[var(--aethel-text-tertiary)]" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 bg-transparent border-none outline-none text-[var(--aethel-text-primary)] text-sm placeholder:text-[var(--aethel-text-tertiary)]"
          />
          <kbd className="px-2 py-0.5 bg-[var(--aethel-surface-quaternary)] rounded text-xs text-[var(--aethel-text-tertiary)]">
            Esc
          </kbd>
        </div>
        ...
        <div ref={listRef} className="overflow-y-auto max-h-[calc(60vh-60px)]">
          {flatCommands.length === 0 ? (
            <div className="px-4 py-8 text-center text-[var(--aethel-text-tertiary)]">
            ...
            <>
              ...
              {recentCommands.length > 0 && (
                <div>
                  <div className="px-4 py-2 text-xs font-semibold text-[var(--aethel-text-tertiary)] uppercase flex items-center gap-2">
        ...
        {/* Footer */}
        <div className="flex items-center gap-4 px-4 py-2 border-t border-[var(--aethel-border-primary)] text-xs text-[var(--aethel-text-tertiary)]">
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 bg-[var(--aethel-surface-quaternary)] rounded text-[10px]">↑</kbd>
            <kbd className="px-1.5 py-0.5 bg-[var(--aethel-surface-quaternary)] rounded text-[10px]">↓</kbd>
            navigate
          </span>
          <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 bg-[var(--aethel-surface-quaternary)] rounded text-[10px]">Esc</kbd>
            close
          </span>
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-2 text-left transition-colors ${
        isSelected ? 'bg-[color-mix(in_srgb,var(--aethel-info)_30%,transparent)]' : 'hover:bg-[var(--aethel-surface-quaternary)]/50'
      }`}
    >
      <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-[var(--aethel-text-tertiary)]">
        {command.icon || CATEGORY_ICONS[command.category]}
      </span>
      
      <div className="flex-1 min-w-0">
        <div className="text-sm text-[var(--aethel-text-primary)] truncate">{command.label}</div>
        {command.description && (
          <div className="text-xs text-[var(--aethel-text-tertiary)] truncate">{command.description}</div>
        )}
      </div>
      
      {isRecent && (
        <Clock className="w-3 h-3 text-[var(--aethel-text-quaternary)]" />
      )}
      
      {command.keybinding && (
        <kbd className="flex-shrink-0 px-2 py-0.5 bg-[var(--aethel-surface-quaternary)] rounded text-xs text-[var(--aethel-text-tertiary)]">
          {command.keybinding}
        </kbd>
      )}
    </button>
