  ...
  return (
    ...
    >
      ...
      <a
        ...
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-[var(--aethel-surface-secondary)] focus:px-3 focus:py-2 focus:text-white focus:ring-2 focus:ring-[var(--aethel-primary)]"
      >
      ...
      {(authErrorText || billingErrorText) && (
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pt-3 sm:px-6">
          <div className="flex flex-wrap gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-xs text-[var(--aethel-text-secondary)]">
            {authErrorText && (
      ...
      {(entryMission || entrySource) && (
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pt-3 sm:px-6">
          <div className="flex flex-col gap-3 rounded-[24px] border border-white/10 bg-[linear-gradient(135deg,rgba(15,23,42,0.94),rgba(2,6,23,0.9),rgba(14,165,233,0.08))] px-4 py-4 shadow-[0_20px_60px_rgba(2,6,23,0.28)] md:flex-row md:items-center md:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {entrySource ? (
                  <span className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-[var(--aethel-text-secondary)]">
                    origem {entrySource}
                  </span>
                ) : null}
              </div>
            ...
            <div className="flex shrink-0 flex-wrap gap-2">
              {onResumeEntryMission ? (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-2xl bg-[linear-gradient(135deg,rgba(79,70,229,0.95),rgba(14,165,233,0.9))] px-4 py-2 text-sm font-semibold text-white shadow-[0_14px_32px_rgba(56,189,248,0.24)] transition hover:brightness-110"
                >
                  Retomar no AI Chat
                </button>
              ) : null}
              {onDismissEntryIntent ? (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-medium text-[var(--aethel-text-secondary)] transition hover:border-white/20 hover:text-white"
                >
                  Ocultar contexto
                </button>
              ) : null}
