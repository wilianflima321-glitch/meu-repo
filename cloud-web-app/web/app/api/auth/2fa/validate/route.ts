...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = await enforceRateLimit({
      scope: 'auth-2fa-validate',
      key: `${getRequestIp(request)}:${userId}`,
      max: 30,
      windowMs: 15 * 60 * 1000,
      message: 'Too many 2FA validation attempts. Please retry later.',
    })
    ...
    if (!result.valid) {
      ...
      if (session.attempts >= 4) {
        await prisma.pendingTwoFactorSession.delete({ where: { token: sessionToken } })
