...
import {
  ...
  buildAiMarginDrilldownFromAggregates,
  buildAiMarginRecommendations,
  buildAiMarginSnapshot,
  costFromAiLedgerAggregate,
  getDateRange,
  metadataRecord,
  normalizePlan,
  stringFromMetadata,
  type AiLedgerAggregateRow,
  type FinanceMetrics,
} from '@/lib/admin/finance-metrics-model';

const routeLogger = createComponentLogger('admin.finance.metrics');

async function loadAiLedgerAggregates(start: Date, end: Date): Promise<AiLedgerAggregateRow[]> {
  // Block 6G.5 / DEBT-FIN-003 — SQL GROUP BY; never unbounded findMany on AI ledger.
  return prisma.$queryRaw<AiLedgerAggregateRow[]>`
    SELECT
      e."userId" AS "userId",
      u.email AS email,
      u.plan AS plan,
      COALESCE(e.metadata->>'model', e.metadata->>'modelId', 'unknown') AS model,
      COALESCE(
        e.metadata->>'projectId',
        e.metadata->>'workspaceId',
        e.metadata->>'project_id',
        e.metadata->>'workspace_id',
        'unattributed'
      ) AS "workspaceId",
      COUNT(*)::int AS calls,
      COALESCE(SUM(ABS(e.amount)), 0)::float AS "amountAbs",
      COALESCE(SUM(NULLIF(e.metadata->>'inputTokens', '')::double precision), 0)::float AS "inputTokens",
      COALESCE(SUM(NULLIF(e.metadata->>'outputTokens', '')::double precision), 0)::float AS "outputTokens",
      COALESCE(SUM(NULLIF(e.metadata->>'totalTokens', '')::double precision), 0)::float AS "totalTokensMeta",
      COALESCE(
        SUM(
          COALESCE(
            NULLIF(e.metadata->>'costUSD', '')::double precision,
            NULLIF(e.metadata->>'costUsd', '')::double precision,
            0
          )
        ),
        0
      )::float AS "directCostUsd"
    FROM "CreditLedgerEntry" e
    INNER JOIN "User" u ON u.id = e."userId"
    WHERE e."entryType" IN ('ai_chat', 'ai_generation', 'usage')
...
async function handler(req: NextRequest) {
  try {
    ...
    const aiAggregates = await loadAiLedgerAggregates(start, end);
    ...
    const aiMarginDrilldown = buildAiMarginDrilldownFromAggregates({
      rows: aiAggregates,
      totalAICost,
      revenueByUserId,
    });
