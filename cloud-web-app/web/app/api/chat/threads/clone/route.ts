...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitResponse = await enforceRateLimit({
      scope: 'chat-thread-clone-post',
      key: user.userId,
      max: 90,
      windowMs: 60 * 60 * 1000,
      message: 'Too many chat thread clone requests. Please wait before retrying.',
    });
    ...
    const source = await prismaAny.chatThread.findFirst({
    ...
    const newThread = await prismaAny.chatThread.create({
      data: {
        userId: user.userId,
        title,
    ...
    const copied = await copyThreadMessages(prismaAny, { sourceThreadId, targetThreadId: newThread.id, maxMessages });
