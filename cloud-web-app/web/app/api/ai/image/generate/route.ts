  ...
  try {
    ...
    const bridged = await runExpensiveCreativeViaBridge({
      ...
      execute: async (ctx) => {
        ...
        return {
          artifactId: primary,
          previewUrl: primary || undefined,
          empty: images.length === 0 || !primary,
    ...
    if (!bridged.ok) {
      const message = bridged.providerError || ''
    ...
    log.info(`[Image API] Generated ${bridged.data.images.length} image(s)`)

    return NextResponse.json(
      {
        success: true,
        provider,
        images: bridged.data.images,
        metadata: {
          prompt,
          size,
          style,
