...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!bridged.ok) return bridged.response

    log.info(`[Music API] Task created: ${bridged.data.result.taskId}`)
    ...
    return NextResponse.json(
      {
        ...
        task: {
          ...
          checkStatusUrl: `/api/ai/music/status?provider=${provider}&taskId=${bridged.data.result.taskId}`,
        },
        metadata: {
