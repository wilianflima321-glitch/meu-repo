...
import { enforceRouteRateLimit, MARKETPLACE_READ_RATE_LIMIT, MARKETPLACE_WRITE_RATE_LIMIT } from '@/lib/server/route-rate-limit';
...
export async function GET(request: NextRequest) {
  try {
    const rateLimited = await enforceRouteRateLimit({
      req: request,
      capability: 'MARKETPLACE_ASSETS_BROWSE',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimited = await enforceRouteRateLimit({
      req: request,
      capability: 'MARKETPLACE_ASSETS_PUBLISH',
      route: '/api/marketplace/assets:publish',
    ...
    // OMNI-PLAN PILAR 5 ("Lei dos 12%"): optional authoring surface for
    // `MarketplaceItem.gameId` — lets a developer mark this listing (e.g. a
    // skin or weapon) as an in-game purchase for one of their own published
    // games, so a future sale credits that game's dedicated-server cost
    // guard. Ownership is checked against `PublishedGame.authorId` so a
    // developer can't attribute revenue credit to a game they don't own.
    const gameIdInput = typeof body?.gameId === 'string' ? body.gameId.trim() : '';
    let gameId: string | null = null;
    if (gameIdInput) {
      const game = await prisma.publishedGame.findUnique({
        where: { id: gameIdInput },
        select: { id: true, authorId: true },
      });
      if (!game) {
        return NextResponse.json(
          { error: 'GAME_NOT_FOUND', message: 'gameId does not reference a published game' },
          { status: 404 }
        );
      }
      if (game.authorId !== user.userId) {
        return NextResponse.json(
          { error: 'GAME_NOT_OWNED', message: 'You can only attach marketplace listings to your own published games' },
          { status: 403 }
        );
      }
      gameId = game.id;
