    ...
    <div className="relative overflow-hidden rounded-3xl border border-[var(--aethel-border-primary)] bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_72%,transparent)] p-8 shadow-2xl md:col-span-2">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-md text-xs leading-5 text-[var(--aethel-warning)]">
            [HELD] In-app Request Payout removed — transfers run through Stripe Express only.
            Aethel Coins / Treasury payout router not shipped.
          </p>
        </div>
        ...
        {stripeAccountId && payoutsEnabled ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-xl bg-[var(--aethel-success)] px-5 py-2.5 text-sm font-semibold text-black transition hover:brightness-110"
          >
            Open Stripe Express
          ...
          <Link
            href="/marketplace/creator/payout-setup"
            className="inline-flex items-center gap-2 rounded-xl border border-[var(--aethel-border-primary)] px-5 py-2.5 text-sm font-semibold text-[var(--aethel-text-primary)] transition hover:brightness-110"
          >
            Finish payout setup
            <ArrowUpRight className="h-4 w-4" />
          </Link>
