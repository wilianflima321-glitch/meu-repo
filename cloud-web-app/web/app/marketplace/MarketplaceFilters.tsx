...
export function MarketplaceFilters({
  ...
  return (
    <div className="border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_42%,transparent)] p-4 shadow-[0_18px_60px_rgba(2,6,23,0.22)] sm:p-5">
      <input
        type="search"
        placeholder="Search extensions, permissions, or tags..."
        value={searchQuery}
        onChange={(event) => onSearchChange(event.target.value)}
        className="h-12 w-full border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_70%,transparent)] px-4 text-sm text-[var(--aethel-text-primary)] outline-none placeholder:text-[var(--aethel-text-quaternary)] focus:border-[color-mix(in_srgb,var(--aethel-info)_55%,transparent)] focus:ring-2 focus:ring-[color-mix(in_srgb,var(--aethel-info)_18%,transparent)]"
      />
      <details className="mt-3 border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_42%,transparent)] px-3 py-3">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-[180px_minmax(0,220px)]">
          <label className="space-y-1">
            ...
            <select
              ...
              className="h-11 w-full border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_70%,transparent)] px-3 text-sm text-[var(--aethel-text-primary)] outline-none focus:border-[color-mix(in_srgb,var(--aethel-info)_55%,transparent)]"
            >
          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className={` border px-3 py-2 text-[11px] font-bold uppercase tracking-[0.14em] transition ${active ? 'border-[color-mix(in_srgb,var(--aethel-info)_34%,transparent)] bg-[color-mix(in_srgb,var(--aethel-info)_12%,transparent)] text-[var(--aethel-info-light)]' : 'border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_30%,transparent)] text-[var(--aethel-text-secondary)] hover:text-[var(--aethel-text-primary)]'}`}
              >
