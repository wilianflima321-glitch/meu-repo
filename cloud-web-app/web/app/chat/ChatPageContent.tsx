    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <select className="bg-[#1a1a1a] border border-white/10 rounded px-3 py-2 text-sm" value={runtime} onChange={e => setRuntime(e.target.value as any)}>
          ...
          {runtime !== 'advanced' || agentCount === 1 ? (
            <select className="bg-[#1a1a1a] border border-white/10 rounded px-3 py-2 text-sm" value={model} onChange={e => setModel(e.target.value)}>
              {modelOptions.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          ) : (
            <>
              <select className="bg-[#1a1a1a] border border-white/10 rounded px-3 py-2 text-sm" value={roleModels.architect} onChange={e => setRoleModels((s) => ({ ...s, architect: e.target.value }))}>
                {modelOptions.map(m => <option key={`arch:${m}`} value={m}>Arquiteto: {m}</option>)}
              </select>
              <select className="bg-[#1a1a1a] border border-white/10 rounded px-3 py-2 text-sm" value={roleModels.engineer} onChange={e => setRoleModels((s) => ({ ...s, engineer: e.target.value }))}>
                {modelOptions.map(m => <option key={`eng:${m}`} value={m}>Engenheiro: {m}</option>)}
              </select>
              {agentCount === 3 ? (
                <select className="bg-[#1a1a1a] border border-white/10 rounded px-3 py-2 text-sm" value={roleModels.critic} onChange={e => setRoleModels((s) => ({ ...s, critic: e.target.value }))}>
        ...
        <div className="bg-[#0d0d0d] border border-white/10 rounded-lg p-4 min-h-[300px] mb-4 overflow-y-auto max-h-[500px]">
          {history.length === 0 ? (
            <div className="text-gray-500 text-center py-8">Envie uma mensagem para começar</div>
          ) : (
            history.map((m, i) => (
              <div key={i} className="mb-4">
                <div className={`flex gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-lg ${m.role === 'user' ? 'bg-blue-600/20 border border-blue-500/20' : 'bg-gray-800/50 border border-white/5'}`}>
                    <div className="text-xs text-gray-400 mb-1">{m.role === 'user' ? 'Você' : 'Aethel'}</div>
                    ...
                    {m.role === 'assistant' && m.traceId ? (
                      ...
                      >
                        {expandedTraceIds[m.traceId!] ? 'Ocultar detalhes' : 'Ver detalhes'}
                      </button>
                    ) : null}
                  </div>
                </div>
                {m.role === 'assistant' && m.traceId && expandedTraceIds[m.traceId] ? (
                  traceById[m.traceId] ? (
                    ...
                    <div className="bg-black/30 p-3 rounded mt-2 text-sm text-gray-400">
                      Carregando detalhes…
                    </div>
                  )
                ) : null}
        ...
        <div className="flex gap-2">
          <input
            type="text"
            className="flex-1 bg-[#1a1a1a] border border-white/10 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500/50"
