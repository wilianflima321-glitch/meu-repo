...
export default function SettingsCommandCenter({
  ...
  return (
    <div data-settings-command-center="true" className="p-4 sm:p-6">
      <section className="rounded-2xl border border-[var(--aethel-border-primary)] bg-[var(--aethel-panel)] p-5 shadow-[var(--aethel-shadow-xl)]">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={() => onSelectTab('security')} className={`rounded-2xl bg-[var(--aethel-primary)] px-4 py-2.5 text-sm font-semibold text-[var(--aethel-text-inverse)] transition hover:bg-[var(--aethel-primary-dark)] ${CANONICAL_FOCUS}`}>
            Review security
          </button>
          <Link href="/billing" className={`rounded-2xl border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_62%,transparent)] px-4 py-2.5 text-sm font-semibold text-[var(--aethel-text-secondary)] transition hover:text-[var(--aethel-text-primary)] ${CANONICAL_FOCUS}`}>
            Open usage
          </Link>
        </div>
      </section>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        {actionCards.map((card) => (
          <button
            ...
            className={`rounded-2xl border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_46%,transparent)] p-4 text-left transition hover:border-[var(--aethel-border-secondary)] hover:bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_48%,transparent)] ${CANONICAL_FOCUS}`}
          >
      ...
      <details className="mt-3 rounded-2xl border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-primary)_34%,transparent)] px-4 py-3">
        ...
        <button
          ...
          className={`mt-3 block w-full rounded-xl border border-[var(--aethel-border-subtle)] bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_38%,transparent)] p-4 text-left transition hover:border-[var(--aethel-border-secondary)] hover:bg-[color-mix(in_srgb,var(--aethel-surface-secondary)_52%,transparent)] ${CANONICAL_FOCUS}`}
        >
