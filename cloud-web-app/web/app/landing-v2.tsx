...
export default function LandingPageV2() {
  ...
  const handleMagicSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Redireciona para a IDE com o prompt como parâmetro de busca
    router.push(`/ide?prompt=${encodeURIComponent(inputValue)}`)
  }
    ...
    <div className="min-h-screen w-full bg-grid-zinc-700/[0.15] relative flex flex-col items-center justify-center overflow-hidden bg-zinc-950">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4">
        ...
        <p className="mt-4 max-w-2xl text-zinc-400 md:text-lg">
          A plataforma de desenvolvimento unificada. Crie, colabore e publique jogos, aplicativos e automações com o poder da IA.
        </p>
        ...
        >
          <div 
            ...
            <div className="absolute -inset-0.5 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl blur-lg opacity-50 group-hover:opacity-80 transition duration-500"></div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-4 text-zinc-400">
          <span className="cursor-pointer hover:text-white transition-colors" onClick={() => router.push('/ide')}>Ir para o Playground</span>
          <span>•</span>
          <span className="cursor-pointer hover:text-white transition-colors" onClick={() => router.push('/login')}>Fazer Login</span>
          <span>•</span>
          <span className="cursor-pointer hover:text-white transition-colors" onClick={() => router.push('/docs')}>Documentação</span>
        </div>
      </div>
    </div>
  )
