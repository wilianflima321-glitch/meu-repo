...
describe('CW1 consolidation truth matrix', () => {
  it('imports live probes and returns machine-readable rows', () => {
    ...
    const parity3b2 = matrix.rows.find((r) => r.id === 'render.parity.3b2')
    ...
    expect(parity3b2?.path).toContain('frame-parity-harness-3b2')
    expect(parity3b2?.status).toBe('PARTIAL')
    expect(parity3b2?.marketingAllowed).toBe(false)
    expect(parity3b2?.lastEvidence).toMatch(/harnessExists=true/)
