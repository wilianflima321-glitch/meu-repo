...
describe('admin consolidation registry', () => {
  ...
  it('maps every legacy admin surface to one canonical owner area', () => {
    ...
    expect(getAdminRouteNavigationHref('/admin/support')).toBe('/admin/users?legacy=support')
    expect(routes).toContain('/admin/roles')
    expect(getAdminRouteNavigationHref('/admin/roles')).toBe('/admin/users?legacy=roles')
    expect(routes).toContain('/admin/emergency')
    ...
    expect(routes).toContain('/admin/marketplace')
    expect(getAdminRouteNavigationHref('/admin/marketplace')).toBe('/admin/finance?legacy=marketplace')
    expect(routes).toContain('/admin/ai-monitor')
