...
function seedFinanceMocks({ revenueCents = 14_000 } = {}) {
  ...
  prismaMocks.prisma.creditLedgerEntry.findMany.mockResolvedValue([
    {
      id: 'ledger-1',
      amount: -7,
      entryType: 'ai_chat',
...
describe('api/admin/finance/metrics route', () => {
  ...
  it('returns an AI margin snapshot using SQL aggregates (6G.5)', async () => {
    const response = await GET(new NextRequest('http://localhost:3000/api/admin/finance/metrics?range=7d'))
    const payload = await response.json()

    expect(response.status).toBe(200)
    expect(prismaMocks.prisma.$queryRaw).toHaveBeenCalled()
  ...
  it('raises a risk snapshot when AI cost is above period revenue', async () => {
    ...
    prismaMocks.prisma.$queryRaw.mockReset()
