...
requireFile('lib/server/studio-workforce-planning.ts', 'studio tasks must share workforce planning inference')
requirePattern('lib/server/studio-workforce-planning.ts', /planAgentWorkforceForMission/, 'studio planning helper must use canonical workforce planning')
requirePattern('app/api/studio/tasks/plan/route.ts', /workforcePlan/, 'studio task plans must persist workforce planning metadata')
requirePattern('app/api/studio/tasks/run-wave/route.ts', /WORKFORCE_HUMAN_APPROVAL_REQUIRED/, 'task waves must hold human-held missions before spawning agents')
requirePattern('lib/server/task-store.ts', /planning\?: Record<string, unknown>/, 'task records must persist planning metadata')
...
requireFile('__tests__/api/studio-task-workforce-routes.test.ts', 'studio task workforce route tests must exist')
requirePattern('__tests__/api/studio-task-workforce-routes.test.ts', /WORKFORCE_HUMAN_APPROVAL_REQUIRED/, 'studio tests must cover human-held wave blocking')
