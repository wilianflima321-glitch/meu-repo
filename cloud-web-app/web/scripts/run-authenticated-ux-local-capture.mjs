...
const PORT = Number(process.env.AUTHENTICATED_UX_LOCAL_PORT ?? 3064)
const HOST = process.env.AUTHENTICATED_UX_LOCAL_HOST ?? '127.0.0.1'
const BASE_URL = `http://${HOST}:${PORT}`
const SERVER_TIMEOUT_MS = Number(process.env.AUTHENTICATED_UX_SERVER_TIMEOUT_MS ?? 90000)
const HAS_PRODUCTION_BUILD = fs.existsSync(path.join(ROOT, '.next', 'BUILD_ID'))
const SERVER_MODE = process.env.AUTHENTICATED_UX_LOCAL_SERVER_MODE ?? (HAS_PRODUCTION_BUILD ? 'start' : 'dev')
const SHOULD_BUILD =
  SERVER_MODE === 'start' &&
  (process.env.AUTHENTICATED_UX_LOCAL_CAPTURE_BUILD === '1' || !HAS_PRODUCTION_BUILD)
const RUN_STRICT = process.env.AUTHENTICATED_UX_LOCAL_STRICT === '1'
const VERBOSE_NEXT_LOGS = process.env.AUTHENTICATED_UX_LOCAL_VERBOSE === '1'
...
const jwtSecret =
  process.env.JWT_SECRET && process.env.JWT_SECRET !== 'your-secret-key-change-in-production'
    ...
    : `aethel-local-ux-${crypto.randomBytes(16).toString('hex')}`
...
async function main() {
  if (SHOULD_BUILD) {
    console.log('[authenticated-ux-local-capture] building production artifact')
    await run(NODE_BIN, [NPM_CLI, 'run', 'build:production'])
  }

  if (SERVER_MODE !== 'start' && SERVER_MODE !== 'dev') {
    throw new Error(`Unsupported AUTHENTICATED_UX_LOCAL_SERVER_MODE=${SERVER_MODE}; expected start or dev.`)
  }

  console.log(`[authenticated-ux-local-capture] starting Next ${SERVER_MODE} server at ${BASE_URL}`)
  const serverArgs =
    SERVER_MODE === 'start'
  ...
  try {
    await waitForServer()
    if (serverExited) throw new Error('Next server exited before capture.')

    await run(commandName('node'), ['scripts/capture-authenticated-ux-surfaces.mjs'], { env: baseEnv })

    if (RUN_STRICT) {
      await run(commandName('node'), ['scripts/check-authenticated-visual-regression.mjs'], {
        env: {
          ...baseEnv,
          AUTH_VISUAL_REGRESSION_STRICT: '1',
        },
      })
    }

    console.log(`[authenticated-ux-local-capture] PASS mode=${SERVER_MODE} baseUrl=${BASE_URL} strict=${RUN_STRICT}`)
  } finally {
    if (!server.killed) {
      server.kill('SIGTERM')
      setTimeout(() => {
...
main().catch((error) => {
  console.error(`[authenticated-ux-local-capture] FAIL ${error instanceof Error ? error.message : String(error)}`)
  process.exit(1)
})
