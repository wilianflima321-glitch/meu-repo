...
requirePattern('lib/runtime/runtime-engine-spine.ts', /zig-toolchain/, 'Zig native compiler must be in the optional build toolchain')
requirePattern('lib/runtime/runtime-engine-spine.ts', /zig-c-compiler/, 'Zig cc wrapper must be modeled for native adapters')
...
requirePattern(studioLocalContracts, /ZigToolchain/, 'Studio Local contracts must model Zig native compiler support')
requirePattern(studioLocalContracts, /ZigCCompiler/, 'Studio Local contracts must model Zig cc wrapper support')
...
requirePattern(studioLocalProbe, /zig/, 'Studio Local probe must detect Zig native compiler support')
requirePattern(studioLocalProbe, /zig-cc/, 'Studio Local probe must detect Zig cc wrapper support')
...
requirePattern('scripts/probe-local-runtime-toolchain.mjs', /zig-toolchain/, 'local runtime probe must report Zig as a governed native build tool')
requirePattern('scripts/probe-local-runtime-toolchain.mjs', /missing > 0 \|\| review > 0/, 'local runtime probe must fail on unmodeled missing or review tools')
requireFile('docs/RUNTIME_TOOLCHAIN_READINESS.md', 'runtime toolchain readiness policy must be documented')
requirePattern('docs/RUNTIME_TOOLCHAIN_READINESS.md', /Held By Design/, 'runtime readiness docs must distinguish held tools from missing tools')
