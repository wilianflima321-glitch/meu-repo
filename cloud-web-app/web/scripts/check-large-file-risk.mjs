...
3. \`lib/mcp/aethel-mcp-server.ts\`: split tool definitions, auth policy, handlers, and response schemas.
4. \`lib/level-serialization.ts\`: decide whether it becomes the canonical Level Editor serializer or move it behind an explicit legacy/archive boundary.
5. \`app/api/ai/chat-advanced/route.ts\` and \`app/api/ai/change/apply/route.ts\`: move policy and apply orchestration into tested production modules.
6. \`lib/pixel-streaming/**\`: keep the new signaling/session/codec/cost split enforced by \`qa:pixel-streaming-split\`.\n\n## Validation\n\nRun \`npm run qa:large-file-risk\` to fail on unbounded file growth or new untracked monoliths.\nRun \`npm run qa:engine-spine-modules\` to ensure V18-V20 engine assets have explicit load strategies before wiring.\n`
}
