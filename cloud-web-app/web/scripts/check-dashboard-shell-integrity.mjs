...
if (lines > MAX_LINES) {
  fail(`AethelDashboardRuntime.tsx regression: ${lines} lines (max ${MAX_LINES})`)
}

if (content.includes("@xyflow/react")) {
  fail('AethelDashboardRuntime.tsx should not import @xyflow/react directly (keep canvas runtime isolated)')
}

console.log(`[qa:dashboard-shell] OK - AethelDashboardRuntime.tsx=${lines} lines`)
