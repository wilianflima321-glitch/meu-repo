...
requirePattern('package.json', /qa:governed-runtime-jobs && npm run qa:runtime-job-receipts && npm run qa:(?:runtime-execution-evidence-package && npm run qa:)?asset-quality-job-runner && npm run qa:studio-local-cook-queue && npm run qa:studio-local-cook-dispatch && npm run qa:production-release-guard && npm run qa:runtime-engine-spine/, 'enterprise gate ordering')
