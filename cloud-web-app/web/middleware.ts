...
function withSecurityHeaders(res: NextResponse, req?: NextRequest, requestId?: string): NextResponse {
  ...
  // Law I (bk) — COOP/COEP on play/runtime/studio/ide so SharedArrayBuffer can enable.
  if (req && pathNeedsCoopCoep(req.nextUrl.pathname)) {
    for (const { key, value } of getCoopCoepHeaderPairs()) {
      res.headers.set(key, value);
    }
  }
