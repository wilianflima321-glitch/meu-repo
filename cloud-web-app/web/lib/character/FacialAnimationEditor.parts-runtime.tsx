...
import {
  BLEND_SHAPE_CATEGORIES,
  FACS_ACTION_UNITS,
  VISEMES,
  type BlendShapeValues,
  type EmotionPreset,
  type LipSyncKeyframe,
} from '@/components/character/facial-animation-model'
...
export const BlendShapeSlider: React.FC<BlendShapeSliderProps> = React.memo(({ name, value, onChange }) => {
...
export const EmotionPresetButton: React.FC<EmotionPresetButtonProps> = React.memo(({ preset, isActive, onClick }) => {
  return (
    <button type="button" aria-label={`Apply emotion preset ${preset.name}`}
      onClick={onClick}
...
export const AudioWaveform: React.FC<AudioWaveformProps> = React.memo(({ audioData, duration, currentTime, onSeek }) => {
...
export const LipSyncTimeline: React.FC<LipSyncTimelineProps> = React.memo(({
  keyframes,
  duration,
  currentTime,
  onAddKeyframe,
  ...
  return (
    <div className="bg-[var(--aethel-surface-secondary)] rounded-lg p-4">
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {VISEMES.map(viseme => (
          <button type="button" aria-label={`Select viseme ${viseme.label}`}
            key={viseme.id}
...
export const FACSReference: React.FC<FACSReferenceProps> = React.memo(({ onSelectAU }) => {
          ...
          >
            ...
            {expandedAU === au.au && (
              <div className="mt-2 pl-2 border-l-2 border-[var(--aethel-border-primary)]">
                ...
                <button type="button" aria-label={`Apply action unit ${au.name} to sliders`}
                  onClick={(e) => {
                    e.stopPropagation()
...
export const FacePreview3D: React.FC<FacePreview3DProps> = React.memo(({ blendShapes }) => {
  return (
