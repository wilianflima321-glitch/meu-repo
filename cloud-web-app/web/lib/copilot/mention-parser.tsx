...
export interface MentionWorkspaceIndex {
  files?: string[]
  functions?: string[]
  symbols?: string[]
}
...
export class MentionSuggestionProvider {
  ...
  constructor(workspace?: MentionWorkspaceIndex) {
    this.loadWorkspaceIndex(workspace)
  }

  private loadWorkspaceIndex(workspace?: MentionWorkspaceIndex) {
    this.files = workspace?.files?.filter(Boolean) ?? []
    this.functions = workspace?.functions?.filter(Boolean) ?? []
    this.symbols = workspace?.symbols?.filter(Boolean) ?? []
  }
...
export { MentionChip, SuggestionList } from './mention-parser.components'
