...
import { FILESYSTEM_LANGUAGE_MAP, FILESYSTEM_MIME_TYPES } from './filesystem-runtime.catalog';
import type {
  CopyOptions,
  DirectoryListing,
  FileChange,
...
export class FileSystemRuntime extends EventEmitter {
  ...
  async getFileInfo(filePath: string): Promise<FileInfo> {
    ...
    return {
      ...
      mimeType: FILESYSTEM_MIME_TYPES[ext] || undefined,
    };
  }
  ...
  async readFile(filePath: string, options?: {
    ...
    return {
      ...
      size: stats.size,
      modified: stats.mtime,
      language: FILESYSTEM_LANGUAGE_MAP[ext],
    };
  }
  ...
  getMimeType(filePath: string): string {
    ...
    return FILESYSTEM_MIME_TYPES[ext] || 'application/octet-stream';
  }
  ...
  getLanguage(filePath: string): string | undefined {
    const ext = path.extname(filePath).toLowerCase();
    return FILESYSTEM_LANGUAGE_MAP[ext];
  }
}
