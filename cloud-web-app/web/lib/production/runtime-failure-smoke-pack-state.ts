import type { RuntimeFailureSmokePackReport } from '@aethel/runtime/runtime-failure-smoke-pack'
...
function packEvidenceRefs(report: RuntimeFailureSmokePackReport): string[] {
  return unique([
    ...
    ...report.scenarios.map((scenario: any) => `runtime-failure-smoke:${scenario.runId}`),
    ...report.scenarios.flatMap((scenario: any) => scenario.ledger.events.flatMap((event: any) => event.evidenceRefs)),
  ], 120)
}
