...
import { RUNTIME_ENGINE_TOOLCHAIN_REGISTRY } from '@/lib/runtime/runtime-engine-spine'
...
const TOOL_IDS = RUNTIME_ENGINE_TOOLCHAIN_REGISTRY.map((tool) => tool.id)
...
function requiredCapabilitiesForTools(tools: string[]): RuntimeCapabilityKey[] {
  return uniqueRuntimeCapabilities(tools.map((tool) => TOOL_CAPABILITY_MAP[tool]).filter((tool): tool is RuntimeCapabilityKey => Boolean(tool)))
}
...
export function coerceStudioLocalCookJobRequest(input: unknown): StudioLocalCookJobRequest | null {
  ...
  return {
    ...
    availableTools: pickArray(input.availableTools).filter((tool) => TOOL_IDS.includes(tool)),
...
export function buildStudioLocalCookQueuePlan(input: {
  ...
  const missingTools = requiredTools.filter((tool) => !input.request.availableTools.includes(tool))
