import type { RuntimeFailureSmokeBrowserRunnerReport } from '@/lib/runtime/runtime-failure-smoke-runner-report'
import { validateRuntimeFailureSmokeBrowserRunnerReport } from '@/lib/runtime/runtime-failure-smoke-runner-report'
...
function browserRunnerEvidenceRefs(report: RuntimeFailureSmokeBrowserRunnerReport): string[] {
  return unique([
    ...
    ...report.evidenceRefs,
    ...report.results.flatMap((result) => result.evidenceRefs),
  ], 160)
}
...
export function summarizeRuntimeFailureSmokeBrowserRunnerReport(
  ...
  return {
    ...
    screenshotRefs: unique(report.results.map((result) => result.screenshot)),
  }
}
