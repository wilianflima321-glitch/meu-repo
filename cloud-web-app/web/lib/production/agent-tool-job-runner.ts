...
export interface GovernedAgentToolJobDecision {
  ...
  architectureLaws?: ArchitectureLawsGateResult
}
...
export function evaluateGovernedAgentToolJob(
  ...
  const enforceLaws = input.enforceArchitectureLaws === true || (isMutating && input.architectureContext !== undefined)
  if (enforceLaws || input.architectureContext) {
    architectureLaws = evaluateArchitectureLawsGate({
      projectId: input.projectId,
      intent: isMutating ? 'apply' : 'read',
      lawsPackId: input.architectureContext?.lawsPackId,
      cartographyManifestId: input.architectureContext?.cartographyManifestId,
      contextPackId: input.architectureContext?.contextPackId,
      projectMemoryDigestId: input.architectureContext?.projectMemoryDigestId,
    })
  }
...
/**
 * Focus 1B — evaluate tool job then fail-closed if targetPaths are not on real disk tree.
 */
export async function evaluateGovernedAgentToolJobWithDiskAuthority(
  input: GovernedAgentToolJobInput & {
    userId: string
    /** Default true for mutating tools */
    enforceDiskPaths?: boolean
    requirePathsExist?: boolean
  },
): Promise<GovernedAgentToolJobDecision> {
  const { enforceAgentTargetPathsOrDeny } = await import('./workspace-tree-authority')
  const base = evaluateGovernedAgentToolJob(input)
  const mutatingTools = ['diff-proposal', 'apply-write', 'git-commit', 'create-level']
  const isMutating = mutatingTools.includes(input.toolId as string)
  const enforceDisk = input.enforceDiskPaths ?? isMutating

  if (!enforceDisk || !input.targetPaths?.length) {
    return base
  }

  const disk = await enforceAgentTargetPathsOrDeny({
    userId: input.userId,
    projectId: input.projectId,
    targetPaths: input.targetPaths,
    requireExists: input.requirePathsExist,
  })

  if (disk.ok) {
    return {
      ...base,
      ledger: appendTaskEvidence(base.ledger, {
        kind: 'read-receipt',
        title: 'Disk path authority PASS',
        summary: `${disk.check.resolved.length} target path(s) verified on real workspace tree`,
        refs: input.targetPaths,
        actor: input.agent,
      }),
    }
  }
  ...
  const blockers = unique([
    ...base.blockers,
    `${disk.code}: ${[...disk.check.missing, ...disk.check.outsideScope].join(', ') || 'empty targetPaths'}`,
  ])
  const ready = false
  return {
    ...base,
    ready,
    allowed: base.enforcement === 'enforced' ? false : true,
    blockers,
    ledger: appendTaskEvidence(base.ledger, {
      kind: 'validation',
      title: 'Disk path authority BLOCK',
      summary: blockers[blockers.length - 1] ?? disk.code,
      refs: input.targetPaths,
      actor: input.agent,
    }),
  }
