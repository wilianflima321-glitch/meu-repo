...
import { asDebugPayload, getErrorMessage } from './debug-adapter-utils';
import { getCapabilitiesForDebugType } from './debug-adapter-capabilities';
import type {
  Breakpoint,
  Capabilities,
  ConsoleMessage,
...
export class DebugAdapter extends EventEmitter {
  ...
  async initialize(): Promise<Capabilities> {
    ...
    this.capabilities = getCapabilitiesForDebugType(this.config.type);
