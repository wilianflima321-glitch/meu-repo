...
export class LocomotionPreset {
  ...
  /** Letter bu — stumble-tagged clip for GAS impact → MM biological bridge. */
  private generateStumbleAnimation(): void {
    const duration = 0.6;
    const frameRate = 30;
    this.motionSystem.addAnimation(
      'stumble_forward',
      ...
      frameRate,
      false,
      ['locomotion', 'stumble', 'recovery'],
      true,
      (time: number) => {
        const t = time / duration;
        const lean = Math.sin(t * Math.PI) * 0.25;
        const transforms = this.createStandingPose(0);
        transforms.set('Hips', {
          position: new THREE.Vector3(0, 1.0 - lean * 0.15, lean * 0.2),
          rotation: new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), lean),
        });
        transforms.set('LeftFoot', {
          position: new THREE.Vector3(-0.12, 0, -lean * 0.1),
          rotation: new THREE.Quaternion(),
        });
        transforms.set('RightFoot', {
          position: new THREE.Vector3(0.12, 0, lean * 0.15),
          rotation: new THREE.Quaternion(),
        });
        return {
          boneTransforms: transforms,
          rootPosition: new THREE.Vector3(0, 0, lean * 0.3),
          rootRotation: new THREE.Quaternion(),
        };
      },
    );
