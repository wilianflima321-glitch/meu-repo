...
import { enableRadianceOnRenderer } from '@/lib/radiance/radiance-viewport-enable';
import { enableFsrOnRenderer } from '@/lib/hardware/fsr-frame-wire';
import { probeWebHardwareProfile } from '@aethel/engine/render/hardware-profile';
...
export function useRenderPipeline(options: UseRenderPipelineOptions = {}): UseRenderPipelineReturn {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Letter cf — real enableRadiance caller (studio 3D pipeline); Law XV probe + Zero-UI degrade.
      const hw = probeWebHardwareProfile({
        webgpuAvailable: typeof navigator !== 'undefined' && 'gpu' in navigator,
        webgl2Available: renderer.capabilities.isWebGL2,
      });
      enableRadianceOnRenderer(aaaRendererRef.current, {
        capabilityScore: hw.capabilityScore,
        radianceRequested: true,
      });
      // Letter ci — CapScore FSR spatial upscale into composer → Present.
      enableFsrOnRenderer(aaaRendererRef.current, {
        capabilityScore: hw.capabilityScore,
        fsrRequested: true,
      });
