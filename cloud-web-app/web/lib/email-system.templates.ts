...
export const EmailTemplates: Record<EmailTemplate, TemplateConfig> = {
  ...
  // Block 6H.8 — billing thresholds (EN — product UI language)
  usage_pool_80: {
    subject: (data) =>
      `${data.poolLabel || 'AI'} pool is at ${data.poolPercent ?? 80}%+ — Aethel`,
    html: (data) =>
      wrapInLayout(
        `
      <h2>${data.poolLabel || 'AI'} pool at ${data.poolPercent ?? 80}%+</h2>
      <p>Hi ${data.name},</p>
      <p>Your included <strong>${data.poolLabel || 'AI'}</strong> pool for ${data.periodKey || 'this month'} is at 80% or more.</p>
      <p>Your IDE stays open. To keep AI flowing:</p>
      <ul>
        <li>Buy wallet credits</li>
        <li>Enable pay-as-you-go with a mandatory spend cap</li>
        <li>Or connect BYOK for your own keys</li>
      </ul>
      <p style="text-align: center; margin: 30px 0;">
        <a href="${data.usageUrl}" class="button">Open Billing &amp; Usage</a>
      </p>
    `,
        data,
      ),
    text: (data) =>
      `Hi ${data.name},\n\nYour ${data.poolLabel || 'AI'} pool is at ${data.poolPercent ?? 80}%+ for ${data.periodKey || 'this month'}.\nIDE stays open — buy credits, enable PAYG with a spend cap, or connect BYOK.\n\n${data.usageUrl}`,
  },

  payg_cap_50: {
    subject: (data) => `PAYG at 50% of your $${data.capUsd} spend cap`,
    html: (data) =>
      wrapInLayout(
        `
      <h2>Pay-as-you-go at 50% of spend cap</h2>
      <p>Hi ${data.name},</p>
      <p>You have accrued <strong>$${data.accruedUsd}</strong> of your <strong>$${data.capUsd}</strong> PAYG spend cap for ${data.periodKey || 'this period'}.</p>
      <p>No surprise bills — at 100% of the cap, post-quota AI pauses until you raise the cap, top up wallet, or wait for the next period.</p>
      <p style="text-align: center; margin: 30px 0;">
        <a href="${data.usageUrl}" class="button">Review PAYG settings</a>
      </p>
    `,
        data,
      ),
    text: (data) =>
      `Hi ${data.name},\n\nPAYG accrued $${data.accruedUsd} of $${data.capUsd} for ${data.periodKey || 'this period'}.\nReview settings: ${data.usageUrl}`,
  },

  payg_cap_100: {
    subject: (data) => `PAYG spend cap reached ($${data.capUsd})`,
    html: (data) =>
      wrapInLayout(
        `
      <h2>PAYG spend cap reached</h2>
      <p>Hi ${data.name},</p>
      <p>Your pay-as-you-go accrual hit the <strong>$${data.capUsd}</strong> spend cap for ${data.periodKey || 'this period'} (accrued $${data.accruedUsd}).</p>
      <p>Post-quota AI is paused. Raise the cap, buy wallet credits, or wait for the next billing period. Your IDE stays unlocked.</p>
      <p style="text-align: center; margin: 30px 0;">
        <a href="${data.usageUrl}" class="button">Manage billing</a>
      </p>
    `,
        data,
      ),
    text: (data) =>
      `Hi ${data.name},\n\nPAYG spend cap of $${data.capUsd} reached (accrued $${data.accruedUsd}).\nManage billing: ${data.usageUrl}`,
  },
};
