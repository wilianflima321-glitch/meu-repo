...
/**
 * CLOTH-001 — numeric spatial hash (no string `${x},${y},${z}` keys per frame).
 * Uses 32-bit mixed cell coords → Map&lt;number, number[]&gt;.
 */
export class SelfCollisionHandler {
  private enabled: boolean = true;
  private thickness: number = 0.02;
  private hashGrid: Map<number, number[]> = new Map();
  ...
  /** Expose for tests — proves numeric keys (not string). */
  getHashKeyNumeric(position: { x: number; y: number; z: number }): number {
    const x = Math.floor(position.x / this.cellSize);
    const y = Math.floor(position.y / this.cellSize);
    const z = Math.floor(position.z / this.cellSize);
    // Knuth multiplicative hash mix — stable Int32-ish key
  ...
  private buildHashGrid(particles: ClothParticle[]): void {
    for (const bucket of this.hashGrid.values()) {
      bucket.length = 0;
      this.cellPool.push(bucket);
    }
    this.hashGrid.clear();
    for (let i = 0; i < particles.length; i++) {
      const key = this.getHashKeyNumeric(particles[i].position);
      let bucket = this.hashGrid.get(key);
      if (!bucket) {
        bucket = this.cellPool.pop() ?? [];
        this.hashGrid.set(key, bucket);
      }
      bucket.push(i);
    }
  }
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        for (let dz = -1; dz <= 1; dz++) {
          const key =
            (((cx + dx) * 73856093) ^ ((cy + dy) * 19349663) ^ ((cz + dz) * 83492791)) | 0;
