...
export async function releaseStorageUsage(
  ...
  const removed = Math.max(0, Math.floor(bytesRemoved));
  if (!userId || removed === 0) return;

  // Block 6G.6 / DEBT-FIN-001 — atomic GREATEST(0, storageUsed - n); no RMW race.
  // DEBT-FIN-004 — never treat falsy storageUsed as missing (0 is valid).
  await prisma.$executeRaw`
    UPDATE "User"
    SET "storageUsed" = GREATEST(0, "storageUsed" - ${removed})
    WHERE id = ${userId}
  `;
}
