...
import type { AnomalyCorrection, Lobby, NetworkConfig, NetworkInput, NetworkMessage, NetworkPlayer, PlayerState } from './networking-multiplayer.types';
...
export class NetworkManager {
  ...
  enableRollback(
    ...
    // OMNI-PLAN PILAR 2 (Rubberbanding Anti-Cheat): forward every server
    // anomaly push straight into the rollback instance so a detected
    // exploit is corrected on the very next received message, not on the
    // next unrelated STATE_UPDATE.
    this.client?.on(MessageType.ANOMALY_CORRECTION, (data: unknown) => {
      this.rollback?.applyAnomalyCorrection(data as AnomalyCorrection);
    });
  }
