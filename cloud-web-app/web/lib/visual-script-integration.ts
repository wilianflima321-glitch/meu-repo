...
import { VisualScriptRuntime, RuntimeContext, Vector3 } from '../../packages/visual-scripting/VisualScriptRuntime';
import type { VisualScript } from '../../packages/visual-scripting/VisualScriptEditor';
...
export class VisualScriptSystem implements System {
  ...
  private createRuntimeContext(entity: Entity, deltaTime: number): RuntimeContext {
    ...
    return {
      ...
      input: {
        getKey: (key: any) => this.inputManager.getKey(key),
        getKeyDown: (key: any) => this.inputManager.getKeyDown(key),
        getKeyUp: (key: any) => this.inputManager.getKeyUp(key),
        getAxis: (axis: any) => this.inputManager.getAxis(axis),
        mousePosition: this.inputManager.mousePosition,
        mouseDelta: this.inputManager.mouseDelta,
        mouseButton: (btn: any) => this.inputManager.mouseButton(btn),
      },
      
      physics: {
        raycast: (origin: any, direction: any, distance: any) => this.performRaycast(origin, direction, distance),
        addForce: (target: any, force: any, impulse: any) => this.applyForce(target, force, impulse),
      },
