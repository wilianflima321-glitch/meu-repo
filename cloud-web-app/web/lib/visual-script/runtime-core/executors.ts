...
import type { VisualScriptRuntime } from './runtime';
import type { ExecutionContext, NodeDefinition, Vector3 } from './types';
import { getGameplayEffect } from '@/lib/gas/effect-registry';
import { getDefaultGasWorld, getOrCreateEntityForTargetId, tickDefaultGasWorld } from '@/lib/gas/visual-script-bridge';
...
nodeExecutors.set('event-update', async (node, inputs, context) => {
  // Best-effort GAS integration point — see `lib/gas/visual-script-bridge.ts#tickDefaultGasWorld`'s
  // doc comment for why this is "best effort" (a catalog-authored
  // `event_update` node, underscore, does not currently reach this
  // hyphenated executor at all — a pre-existing mismatch in this file
  // unrelated to GAS, not fixed here).
  tickDefaultGasWorld(context.deltaTime);
...
// ============================================================================
// ABILITY NODES (OMNI-PLAN GAS — Data-Oriented Gameplay Ability System)
// ============================================================================
//
// These three executors are registered under the SAME underscore-prefixed
// type strings used in `visual-node-catalog.ts` (`ability_apply_effect`,
// `ability_check_tag`, `ability_wait_delay`), unlike most of the executors
// above (`action-move`, `flow-delay`, ...) which use hyphens that do not
// actually match their `visual-node-catalog.ts` counterparts
// (`action_move`, `flow_delay`, ...) — a pre-existing inconsistency between
// this runtime and the editor's node catalog, not introduced or fixed here.
// Using the catalog's exact string for these new nodes means a graph
// authored in the real Visual Scripting editor executes them correctly.

nodeExecutors.set('ability_apply_effect', async (node, inputs, context, runtime) => {
  const targetId = inputs.get('target') as string || '';
  const sourceId = inputs.get('source') as string || '';
  const effectId = (inputs.get('effectId') as string) || (node.data?.effectId as string) || '';

  const outputs = new Map<string, unknown>();
  outputs.set('applied', false);

  const definition = getGameplayEffect(effectId);
  if (!targetId || !definition) {
    log.info(`[GAS] ability_apply_effect: unknown effectId "${effectId}" or missing target — no-op`);
    return { outputs, nextExec: 'exec' };
  }

  const targetEntity = getOrCreateEntityForTargetId(targetId);
  const sourceEntity = sourceId ? getOrCreateEntityForTargetId(sourceId) : undefined;
  const applied = getDefaultGasWorld().applyGameplayEffect(targetEntity, definition, sourceEntity);
  outputs.set('applied', applied);

  runtime.emit('gas:effect_applied', { targetId, effectId, applied });

  return { outputs, nextExec: 'exec' };
});

nodeExecutors.set('ability_check_tag', async (node, inputs) => {
  const targetId = inputs.get('target') as string || '';
  const tag = inputs.get('tag') as string || (node.data?.tag as string) || '';

  const outputs = new Map<string, unknown>();
  if (!targetId || !tag) {
    outputs.set('hasTag', false);
    return { outputs };
  }

  const targetEntity = getOrCreateEntityForTargetId(targetId);
  outputs.set('hasTag', getDefaultGasWorld().hasTag(targetEntity, tag));

  return { outputs };
});

nodeExecutors.set('ability_wait_delay', async (node, inputs) => {
  const duration = (inputs.get('duration') as number) ?? (node.data?.duration as number) ?? 1;

  await new Promise((resolve) => setTimeout(resolve, Math.max(0, duration) * 1000));
