...
**Purpose**: This document is the single source of truth for one complete execution pass. The next agent (Claude) must implement **everything** described below in a single, continuous session without asking for clarification. All decisions, constraints, risks, and quality bars are explicitly defined.

**Date**: June 27, 2026  
**Status**: Definitive. Supersedes all previous roadmaps and backlogs.
...
**Requirements**:
- Replace `memStore` with `@upstash/redis` (preferred) or `ioredis`.
- Use atomic `hincrby` + `expire` for daily spend keys.
