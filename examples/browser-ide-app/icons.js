 ...
 * SVG icons for enterprise-grade UI
 * Inspired by Lucide Icons
 */
...
const Icons = {
  ...
  file: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>`,
  ...
  stop: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>`,
  ...
  // Tools
  settings: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M12 1v6m0 6v6m5.2-13.2-4.2 4.2m0 6 4.2 4.2M23 12h-6m-6 0H1m18.2-5.2-4.2 4.2m0 6 4.2 4.2"></path></svg>`,
...
class IconSystem {
   ...
   * @param {object} options - Options (size, color, class)
  ...
  static create(name, options = {}) {
    ...
    wrapper.innerHTML = this.get(name, options);
    return wrapper;
  }
  ...
  static replaceEmojis(element) {
    ...
    for (const [emoji, iconName] of Object.entries(emojiMap)) {
      if (text.includes(emoji)) {
        const icon = this.get(iconName, { size: 20 });
        element.innerHTML = element.innerHTML.replace(emoji, icon);
      }
    }
  }
  
  /**
   * Replace all emojis in document
   */
  static replaceAllEmojis() {
    const elements = document.querySelectorAll('button, a, .folder-item, .filter-item, .asset-type-icon');
    elements.forEach(el => this.replaceEmojis(el));
  }
}

// Export for use
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { Icons, IconSystem };
}
