...
- Workbench hotspots:
- `cloud-web-app/web/components/ide/FullscreenIDE.tsx` esta em `379` linhas e segue como a shell mais densa do cockpit.
- `cloud-web-app/web/components/ide/fullscreen/FullscreenIDEWorkspaceBridge.tsx` caiu para `89` linhas e a densidade do route/workspace bridge foi empurrada para `useFullscreenIDEBridgeSections.ts` (`141`) e `FullscreenIDEWorkspaceBridge.types.ts` (`117`), com `useFullscreenIDEBridgeProps.ts` agora em `19` linhas.
...
- a lane principal do editor agora esta repartida em `cloud-web-app/web/components/ide/fullscreen/WorkbenchEditorSurface.tsx` (`99` linhas), `cloud-web-app/web/components/ide/fullscreen/WorkbenchEditorCanvas.tsx` (`96` linhas), `cloud-web-app/web/components/ide/fullscreen/WorkbenchEditorToolbar.tsx` (`191` linhas), `cloud-web-app/web/components/ide/fullscreen/WorkbenchEditorSidecar.tsx` (`85` linhas), `WorkbenchEditorSurface.types.ts` (`62`) e `WorkbenchSplitEditorSurface.tsx` (`121`).
- a lane de preview agora esta mais distribuida em seams reais: `cloud-web-app/web/components/preview/CanonicalPreviewSurface.tsx` segue como roteador de `89` linhas, `cloud-web-app/web/components/ide/fullscreen/WorkbenchPreviewPane.tsx` caiu para `44` linhas como orchestrator, com a densidade do cockpit movida para `WorkbenchPreviewRuntimeControls.tsx` (`134`), `WorkbenchPreviewRuntimeSurface.tsx` (`87`), `WorkbenchPreviewModeHeader.tsx` (`70`) e `workbenchPreviewPaneModels.ts` (`56`); `cloud-web-app/web/components/preview/SceneViewportSurface.tsx` agora caiu para `98`, apoiado por `SceneViewportStage.tsx` (`117`), `useSceneViewportSurfaceState.ts` (`149`), `useSceneViewportPlayback.ts` (`53`), `SceneViewportWorkflowDrawer.tsx` (`163`), `RuntimePreviewSurface.tsx` (`198`), `PreviewLifecycleChrome.tsx` (`151`), `usePreviewRuntime.ts` (`107`), `previewRuntimeState.ts` (`38`), `usePreviewRuntimeHealthMonitor.ts` (`57`), `usePreviewRuntimeHmrBridge.ts` (`78`) e `sceneViewportDerivations.ts` (`89`).
- `cloud-web-app/web/components/ide/ModernIDEShell.tsx` ja caiu para `149` linhas.
- o shell agora esta repartido entre `cloud-web-app/web/components/ide/modern-shell/ModernIDEShellPanels.tsx` (`114` linhas), `ModernIDEShellCenterStack.tsx` (`109`), `ModernIDEShellChrome.tsx` (`29`), `ModernIDEShellSideColumns.tsx` (`8`), `chromeHeader.tsx` (`67`), `chromeResizeHandle.tsx` (`106`), `chromeBottomDock.tsx` (`63`), `chromeStatusBar.tsx` (`47`) e `sideColumnChromeParts.tsx` (`69`).
- `cloud-web-app/web/components/terminal/XTerminal.tsx` segue como barrel de `12` linhas; `BaseXTerminal.tsx` agora e apenas uma shell de `105` linhas, enquanto a densidade real do terminal passou para `cloud-web-app/web/components/terminal/useTerminalRuntime.ts` (`150`), apoiada por `useTerminalTransport.ts` (`145`), `useTerminalSessions.ts` (`120`), `terminalSessionApi.ts` (`71`), `terminalSessionConnection.ts` (`61`), `useTerminalSelection.ts` (`64`), `useTerminalShortcuts.ts` (`51`), `useTerminalViewport.ts` (`66`), `useTerminalOptions.ts` (`58`), `useTerminalImperativeHandle.ts` (`51`) e `MultiTerminalPanel.tsx` (`70`).
- Colaboracao:
...
- Build parity:
  ...
  - a árvore pública/auth agora passa por um root shell mínimo em `cloud-web-app/web/components/ClientLayout.tsx` (`17` linhas), enquanto `cloud-web-app/web/components/providers/CoreUiProviders.tsx` concentra `ThemeProvider` + `ToastProvider`, `StudioRuntimeProviders.tsx` sobe o runtime pesado por rota, `app/(auth)/layout.tsx` virou um shell pass-through, `login-v2.tsx` / `register-v2.tsx` montam `CoreUiProviders` só no browser sob `force-dynamic` + `ssr: false`, `app/admin/layout.tsx` agora monta o runtime completo do Studio no admin, `app/design-system-demo/layout.tsx` / `app/verify-email/layout.tsx` montam os providers de UI explicitamente, `cloud-web-app/web/package.json` agora força `NODE_ENV=production` em `build`/`build:analyze`, os `.env*.example` canônicos não mais fixam `NODE_ENV=development`, e `cloud-web-app/web/next.config.js` força `experimental.workerThreads=false` como mitigação para builds Windows.
...
- 🧹 God components: a categoria continua válida, mas os números antigos envelheceram; hoje os maiores seams centrais já são mais concentrados em `FullscreenIDE.tsx` (`379`), `AIChatPanelPro.tsx` (`273`), `usePreviewRuntime.ts` (`213`), `WorkbenchEditorSurface.tsx` (`209`), `RuntimePreviewSurface.tsx` (`198`) e `useTerminalRuntime.ts` (`150`), enquanto a antiga densidade do bridge foi repartida entre `useFullscreenIDEBridgeSections.ts` e `useFullscreenIDEBridgeProps.types.ts`.
...
- `CanonicalPreviewSurface.tsx` já não é mais um god component de quatro dígitos; hoje é um roteador de `89` linhas apoiado por `RuntimePreviewSurface.tsx`, `SceneViewportSurface.tsx`, `PreviewLifecycleChrome.tsx` e os hooks de runtime/viewport.
