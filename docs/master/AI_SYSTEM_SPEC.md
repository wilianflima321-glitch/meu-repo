# AI_SYSTEM_SPEC (LEGACY FILENAME)
...
| **Model Router** | Seleciona e chama modelos | Backend (Next.js API Routes / Node) |
| **Agent Orchestrator** | Gerencia agents e tasks | Backend (Next.js API Routes / Node) |
...
- `EXECUTION_PLAN.md` - Plano de execução completo


---

## EXECUTION DELTA (2026-02-13)

Implemented alignment in current repo:
- L1 inline completion remains editor-native in Monaco (`ghost text` via inline completion provider).
- L2 chat remains available in right AI panel, but Workbench keeps code editing as primary interaction surface.
- L3 quick actions remain bound to editor workflow (`Ctrl+K`) through inline edit/action patterns.
- Provider failures continue to return explicit `NOT_IMPLEMENTED` for non-configured AI backends.
