...
4. Keep `qa:economics-transparency` green so chat cost, wallet, monthly budget, trial state, and billing portal truth stay visible without UI pollution.
5. Add visual tests for Web Entry, Studio Home, IDE preview/review, and device runtime card.
6. Keep all new entry surfaces under the one-protagonist rule.
7. Continue wiring game/film tools by mode instead of making them global navigation.
8. Expand Studio Local from capability snapshot into controlled native worker execution.
9. Add more route-level checks for preview/chat proportions and mobile approval flows.
