...
1. Add visual tests for Web Entry, Studio Home, IDE preview/review, and device runtime card.
2. Keep all new entry surfaces under the one-protagonist rule.
3. Continue wiring game/film tools by mode instead of making them global navigation.
4. Expand Studio Local from capability snapshot into controlled native worker execution.
5. Add more route-level checks for preview/chat proportions and mobile approval flows.
