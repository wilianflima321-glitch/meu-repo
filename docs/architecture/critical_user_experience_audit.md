...
| 11 | VS port magnetism / quick search | UX | 20px snap + fuzzy wire on drop | `IMPROVE-VS-012` |
| 12 | Console logs lost from playtest iframe | **CONFIRMED** — `ConsoleIntegration.tsx` parent only | postMessage / IPC bridge | `IMPROVE-IDE-018` |
| 13 | Console 100 log cap | **CONFIRMED** — `slice(-99)` ~64 | Virtualize 5k logs | `IMPROVE-IDE-019` |
...
| 17 | Dashboard banner stack (not Linear-minimal) | **CONFIRMED** — `DashboardShell.tsx` ~113–151 | Collapse to single intent row | `DEBT-UX-DASH-001` |
...
|---|-------|------------|----------|------------|-----|
| 21 | **AI-tunneling UX** — dashboard forces chat, not workspace | **DONE (verified 2026-06-19)** | `DashboardEntryIntentBanner.tsx` L58 already reads **`Resume Workspace`**; heading "Continue where you left off." | Remaining: persist/restore tabs/panels/scroll from last IDE session | `IMPROVE-UX-009` |
...
| 23 | **Orphan admin routes → 404** | **PARTIAL** — **12** subdirs have panels but no `page.tsx` (user cited 18) | Missing pages: `ai-agents`, `ai-monitor`, `ai-training`, `apis`, `collaboration`, `compliance`, `ide-settings`, `infrastructure`, `marketplace`, `moderation`, `payments`, `roles`; `admin-consolidation.ts` redirects exist but direct URLs 404 | Delete orphan dirs or add thin `page.tsx` re-exports; consolidate nav in Admin Command Center tabs | `DEBT-ADMIN-002` |
