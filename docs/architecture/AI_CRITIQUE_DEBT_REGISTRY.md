...
| `DEBT-YJS-001` | Yjs fallback handler broadcasts updates but never `Y.applyUpdate` on server doc | **M** | **1** | **CLOSED (2026-07-25)** — `applyCollaborationUpdate(roomName, update)` called in `legacy-collaboration-handler.ts` ~71 before broadcasting |
...
| `DEBT-ASSET-001` | `ModelLoader` flattens GLTF/OBJ to single vertex/index buffers — destroys hierarchy, skeleton, materials | **L** | **2** | **PARTIAL CLOSED 2026-07-11ac** — viewport drop path preserves hierarchy via `meshUrl` + `ViewportImportedAssetMesh` (GLTF/FBX/OBJ); USD/USDZ HELD; cook flatten path may remain |
