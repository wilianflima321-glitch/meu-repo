...
**Cursor validation:** **PARTIAL** — not empty placeholder; has `CapabilityProbe`, `JobsLane`, `SidecarManager`, `LocalRuntimeStatus`, `CloudHandoffBridge` (`StudioLocalApp.tsx`). Still **not** a product-grade local workspace shell.
...
| `DEBT-DESK-002` | Desktop terminal **held** — no real PTY/shell spawn (contract by design) | **L** | **2** | **CONFIRMED** — `terminal_create`/`terminal_write` |
| `DEBT-DESK-003` | `fs_watch` captures notify events but discards them — WebView never notified | **M** | **3** | **CONFIRMED** — `Ok(_event) => {}` |
| `DEBT-DESK-004` | `ai_complete` + `notify_native` = static `provider_unavailable` (no ONNX sidecar) | **L** | **2** | **CONFIRMED** |
...
| `DEBT-DESK-005` | Web shell ↔ Rust reactor gap — no zero-copy IPC; scene graph isolated (Three.js vs daemon logs) | **XL** | **2** | **CONFIRMED** — JSON payloads in `desktop_commands.rs` |
...
| `DEBT-SSR-001` | Asset pipeline uses `document`/`URL.createObjectURL` — SSR crash if imported server-side | **M** | **2** | **CONFIRMED** — boundary comment exists, not enforced |
...
| `DEBT-YJS-001` | Yjs fallback handler broadcasts updates but never `Y.applyUpdate` on server doc | **M** | **1** | **CONFIRMED** — `legacy-collaboration-handler.ts` ~38–45 |
...
| `DEBT-SIDECAR-001` | `v29-sidecar-lifecycle` forces `releaseReady: false` + human-review blocker | **S** | **3** | **CONFIRMED by design** — governance gate |
| `DEBT-ASSET-001` | `ModelLoader` flattens GLTF/OBJ to single vertex/index buffers — destroys hierarchy, skeleton, materials | **L** | **2** | **CONFIRMED** — `asset-pipeline-runtime/loaders.ts` ~187–254 |
| `DEBT-TERRAIN-001` | Terrain `sculpt_smooth` brush = identity `(h,_d)=>h` — no separate smooth pass | **S** | **2** | **CONFIRMED** — `TerrainSculptingEditor.runtime.tsx` ~165–166 |
| `DEBT-PERF-003` | Foliage painter — one `<mesh>` per instance, new geometry each; no `InstancedMesh` | **M** | **2** | **CONFIRMED** — `FoliagePainterPanels.runtime.tsx` ~206–254 |
...
| `DEBT-AUDIO-001` | Reverb convolver created but **no source** routed to `reverbNode` input — zones change wet gain only | **M** | **2** | **CONFIRMED** — `spatial-audio-system.ts` + `spatial-audio-manager-core.ts` play() path |
| `DEBT-DESK-006` | `native_kernel.rs` manifest drift — `native-pty-contract` state `Available` vs `desktop_commands` held | **S** | **2** | **CONFIRMED** — contradictory claims |
...
| `DEBT-FOLIAGE-001` | `removeCluster` calls `instancedMesh.clear()` — erases **all** instances of foliage type; `cluster.visible` never applied to GPU | **M** | **1** | **CONFIRMED** — `foliage-system.ts` ~252–259, ~293 |
| `DEBT-CLOUD-001` | Volumetric clouds: no depth blend; `blueNoise` null; DOM `querySelector` per frame; `GodRaysPass` never rendered | **M** | **2** | **CONFIRMED** — `volumetric-clouds.ts` ~65, ~116, ~344–365 |
| `DEBT-MOTION-001` | Motion matching: pose DB heap-heavy (Maps of Vector3/Quaternion); `poses.find` O(N) playback; foot lock = lerp not IK | **L** | **2** | **CONFIRMED** — `motion-matching-system.ts` ~337–349, ~531, ~191 |
...
| `DEBT-DEST-001` | Fracture: 10³ grid pseudo-Voronoi; `cellToGeometry` XZ `atan2` fan; fragments = JS velocity not Rapier | **L** | **2** | **CONFIRMED** — `destruction-fracture-generator.ts`, `destruction-fragment-runtime.ts` ~71–86 |
| `DEBT-CLOTH-001` | Cloth CPU Verlet + `SelfCollisionHandler` string hash keys per frame; `GPUClothSimulation` has no collision/skeleton wiring | **M** | **2** | **CONFIRMED** — `cloth-simulation-collisions.ts` ~156–160; `cloth-simulation-gpu.ts` wind/gravity only |
| `DEBT-AUDIO-002` | `generateVoice()` returns empty `createBuffer` (silence) — lipsync → viseme `sil` | **M** | **2** | **CONFIRMED** — `ai-audio-engine.ts` ~306–317; **nuance:** `ai-audio-engine-sfx.ts` procedural SFX works |
| `DEBT-VR-001` | Foveation shader darkens periphery only; `applyToLayer()` never called in `onXRFrame` | **M** | **2** | **CONFIRMED** — `webxr-vr-foveated-rendering.ts`, `webxr-vr-system-core.ts` ~168–203 |
| `DEBT-TERM-001` | xterm → WS → `node-pty` spawns shell on **Node server host** (cloud container), not user machine; no Tauri/desktop bridge in `components/terminal/*` | **L** | **1** | **CONFIRMED** — `terminalWebSocket.ts` ~28–33; `terminal-pty-runtime.ts` ~185; `terminalSessionApi.ts` → `/api/terminal/create` |
...
| `DEBT-DESK-007` | Parallel Electron templates (`runtime-templates/`) vs canonical Tauri 2 (`apps/studio-local/`) — duplicate packaging, ~100MB vs ~5MB | **M** | **0** | **CONFIRMED** — remove Electron channel |
...
| **Claude action** | Connect browser-operator lane or mark UI `held` with gate |
