...
| Motion matching heap | `lib/motion-matching-system.ts` | Poses = `Map` of cloned `Vector3`/`Quaternion` per bone per frame ~337–349 | 🔴 | `DEBT-MOTION-001` |
| Motion pose lookup | `lib/motion-matching-system.ts` | `database.poses.find(...)` linear ~531–533 on playback path | 🔴 | `DEBT-MOTION-001` → `IMPROVE-ENG-014` |
...
| Foot lock | `lib/motion-matching-system.ts` | `FootLockingIK` lerps foot position ~191 — no two-bone IK | 🔴 | `IMPROVE-ENG-014` |
...
| Destruction fracture | `destruction-fracture-generator.ts` | 10³ grid Voronoi; XZ `atan2` triangulation; JS physics not Rapier | 🔴 | `DEBT-DEST-001` |
| Cloth simulation | `cloth-simulation*.ts` | CPU Verlet; string hash Map GC; GPU cloth no skeleton collision | 🔴 | `DEBT-CLOTH-001` |
