...
| `CinematicTimelinePanel.tsx` | **MISSING** — Sequencer IDE PARTIAL; standalone `packages/ide-ui/Timeline3D.tsx` (752 LoC, over the 500-LoC limit, dozens of hardcoded hex colors in inline styles) is only ever fed a static `duration={8}` demo prop by `CanonicalSequencer.tsx` — **never wired to real scene/animation timeline data** (2026-08-08 creative-tools-ux-audit; not fixed this pass, architectural data-wiring gap not a token swap) | PARTIAL (`sequencerPlayReady`) | No UE Sequencer exceeded; do not claim scrubbing/keyframing works against real project data — it does not today |
...
| `PostProcessSettingsPanel.tsx` | Exists at `cloud-web-app/packages/ide-ui/PostProcessSettingsPanel.tsx` — confirmed **0 importers anywhere in the repo** (2026-08-08 creative-tools-ux-audit) | **HELD** — fully static mock-up, sliders have no `onChange`/connected state, not imported/exported anywhere; a genuine Anti-Mock Doctrine violation if ever surfaced as-is. Deletion vs. rewire left to architect sign-off (same treatment as `MaterialGraphPanel.tsx` below), not removed unilaterally this pass | **No.** §2.17 parity claims are 0% real for this file |
| `MaterialGraphPanel.tsx` | Exists at `cloud-web-app/packages/ide-ui/MaterialGraphPanel.tsx` — confirmed **0 importers anywhere in the repo** (2026-08-08 creative-tools-ux-audit; already flagged as triplication debt in the `2026-07-25 aads` changelog entry) | **HELD** — fully static mock-up with hardcoded nodes/connections, no real graph engine underneath. Real, functional material graph editing lives in `web/lib/materials/MaterialEditor.runtime.tsx` (ReactFlow + live Three.js preview) | **No** |
| AI/Agent chat + diff review (`AIChatPanelPro.tsx`, `AIChatOpsSidebar.tsx`, `AIChatPendingDiffTray.tsx`, `InlineAIChat*`) | **Exists and functional** — Nexus phase strip, ledger receipts, Trava II undo banner, cost meter, per-agent fleet status all wired to real state (2026-08-08 chat-ux-audit) | **PARTIAL vs Cursor/Copilot Chat.** Fixed this pass: `AIChatOpsSidebar` per-file "Approve"/"Reject" was silently applying/rejecting the **wrong** pending diff (`newContent` passed where `filePath` was expected) — Trava II partial-apply was broken, not cosmetic; corrected. Fixed: `useRealtimeVoiceSession`'s `lastBlockedReason` (rate-limit/provider/invalid-input) was captured but never rendered — a failed voice turn failed silently; now surfaced as an alert in the live overlay. **Still real gap (P0, not fixed — needs a dedicated round):** `InlineAIChat`/`useInlineAIChatSession.ts` has **no token-by-token streaming** — full response renders only after the fetch resolves, no incremental text, no mid-flight visible cancel. This is the largest remaining gap vs. Cursor/Copilot Chat/Devin-class chat. `AIChatPendingDiffTray`'s compact strip is intentionally all-or-nothing (file list hidden until "Open diff"); full per-file review exists one click away in `AIChatOpsSidebar`, so this is P1 polish not a blocker. | No claim that inline chat streams "in real time" or is "as fluid as Cursor" until incremental streaming ships. AI-native/J.1+J.2+J.12 marketing claims: none found in audited scope (clean). |
