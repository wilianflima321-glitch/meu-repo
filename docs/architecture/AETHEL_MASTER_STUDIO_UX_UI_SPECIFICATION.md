...
| `CinematicTimelinePanel.tsx` | **MISSING** — Sequencer IDE PARTIAL; standalone `packages/ide-ui/Timeline3D.tsx` (752 LoC, over the 500-LoC limit, dozens of hardcoded hex colors in inline styles) is still demo-fixture keyframes — `CanvasViewportSurface` passes `duration={8} demoMode` and `CanonicalSequencer` defaults to `DEMO_SEQUENCE` (2026-08-08 honesty pass: explicit **Demo timeline / Demo sequence** badges; not marketed as live scene data). Real scene/animation wiring still OPEN | PARTIAL (`sequencerPlayReady`) | No UE Sequencer exceeded; do not claim scrubbing/keyframing works against real project data — it does not today |
