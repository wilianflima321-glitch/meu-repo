    ...
    NativeKernelManifest {
        ...
        capabilities: vec![
            ...
            NativeKernelCapability {
                ...
                label: "GPU-driven frustum/occlusion culling contract",
                state: NativeKernelState::NeedsReview,
                evidence_refs: vec!["src-tauri/src/gpu_culling.rs"],
                blocker: Some("The compute-shader frustum cull is implemented but not wired into the frame render loop's draw call, has no occlusion (Hi-Z) pass yet, and has not been validated against a working GPU/cargo build in this environment."),
                next_action: "Wire GpuCullingPipeline output into an actual draw_indirect call, then add the Hi-Z occlusion pass.",
            },
            NativeKernelCapability {
                ...
                blocker: Some("The wasmtime-based host, state-preserving swap, and file watcher are implemented, but the `wasmtime` dependency version is unpinned and this has not been exercised end-to-end against a real compiled gameplay module in this environment."),
                next_action: "Pin the wasmtime version, compile a real sample gameplay module, and verify a live hot-swap against a running game loop.",
            },
            NativeKernelCapability {
                ...
                blocker: Some("Only the deterministic analytical root-motion fallback is proven. The ONNX Runtime (`ort`) neural evaluator is feature-gated behind `local-ai`, written against the `ort` 2.x API from documentation only, and has not been verified to compile or run against a real trained model."),
                next_action: "Verify the `local-ai` feature builds against the installed `ort` version, then validate inference against a real trained motion-matching ONNX model.",
            },
            NativeKernelCapability {
                ...
                blocker: Some("`ClientConsensusValidator` (speed/acceleration/teleport plausibility checks) is real, deterministic, and unit-tested in isolation — and `physics_kernel.rs`/`physics_commands.rs` were, until this pass, dead files not wired into the crate at all (missing `pub mod physics_kernel;`/`mod physics_commands;` and a managed `Mutex<PhysicsKernel>`), which this pass also fixed. What remains unverified: no headless server binary exists yet to actually run this against live network traffic, and it is a kinematic plausibility check, not the full ML-based anomaly model the brief's phrase \"IA leve\" evokes — see the module doc comment in physics_kernel.rs for the exact scope claim."),
                next_action: "Build the headless Linux target `packages/infra/k8s/agones/fleet.yaml` expects, wire `validate_client_transform` into its per-tick network ingest loop, and push rejections to clients via `MessageType.ANOMALY_CORRECTION`.",
            },
            NativeKernelCapability {
                ...
                blocker: Some("AttributeTable/GameplayTagRegistry/TagSetTable/GameplayEffectPool/GasWorld are structurally complete, batch-process via a rayon par_iter_mut pass (see `GameplayEffectPool::tick`'s doc comment for why only the numeric countdown, not the expiry/pulse application, is parallelized), and carry 8 unit tests (instant/duration/period math, attribute clamping, hierarchical tag matching, blocked-tag rejection, 5,000-effect batch tick) mirroring `cloud-web-app/web/lib/gas/*`'s TypeScript twin exactly, including a shared boundary-ordering fix (period pulse before duration expiry in the same tick) applied to BOTH implementations. What is unverified: no `cargo check` has run against this file (see `apps/studio-local/Dockerfile.rust-builder` — Docker Desktop was unavailable in this environment when this pass tried it), and there is no Tauri IPC surface (`#[tauri::command]`) or headless-server wiring yet — this is a library-only module today."),
                next_action: "Run `docker build -f apps/studio-local/Dockerfile.rust-builder apps/studio-local/src-tauri` (or a native `cargo check`) to validate compilation, then decide whether this Rust GAS replaces or coexists with the TypeScript one in `lib/gas/` for the headless dedicated server path.",
            },
        ],
