...
pub struct RuntimeJobStatus {
    ...
    /// Unix epoch milliseconds. `#[serde(default)]` so job snapshots persisted
    /// before this field existed still deserialize (as `0`) instead of
    /// breaking job recovery on restart.
    #[serde(default)]
