...
function processKernelFile(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        let line = lines[i];
        if (line.match(/^[ \t]*pub distinct_from_[a-zA-Z0-9_]+(?:\s*:\s*bool)?\,/)) {
            if (!addedNoteDef) {
                lines[i] = line.replace(/pub distinct_from_[a-zA-Z0-9_]+(?:\s*:\s*bool)?\,/, 'pub distinct_from_peers_note: String,');
                addedNoteDef = true;
            } else {
                lines[i] = null;
            }
    ...
    // 2. Struct instantiations (handles distinct_from_x: val, AND distinct_from_x,)
    lines = content.split('\n');
    let addedNoteInst = false;
    for (let i = 0; i < lines.length; i++) {
        let line = lines[i];
        if (line.match(/^[ \t]*distinct_from_[a-zA-Z0-9_]+(?:\s*:\s*[^,]+)?\,/)) {
            // But wait! If it's r.distinct_from_... that was already handled or is a mutation.
            // This is just distinct_from_something,
            if (!addedNoteInst) {
                // Ensure it retains indentation
                const indentMatch = line.match(/^([ \t]*)/);
                const indent = indentMatch ? indentMatch[1] : '        ';
                lines[i] = indent + 'distinct_from_peers_note: "HELD: Distinct from many peers. Fingerprint cross-check held to avoid coupling.".to_string(),';
                addedNoteInst = true;
            } else {
                lines[i] = null;
            }
    ...
    const letDistinctRegex = /let distinct_from_[a-zA-Z0-9_]+\s*=\s*[^;]+;/g;
...
function processWireFile(filePath) {
    ...
    // Convert to `.clone()` for String ownership in Tauri wire layer (since r is a reference or the struct owns it, we clone the string to be safe)
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.match(/^[ \t]*distinct_from_[a-zA-Z0-9_]+:\s*r\.distinct_from_[a-zA-Z0-9_]+/)) {
            if (!addedNoteMap) {
                lines[i] = line.replace(/distinct_from_[a-zA-Z0-9_]+:\s*r\.distinct_from_[a-zA-Z0-9_]+(?:.clone\(\))?\,/, 'distinct_from_peers_note: r.distinct_from_peers_note.clone(),');
                addedNoteMap = true;
            } else {
                lines[i] = null;
            }
        ...
        if (line.match(/^[ \t]*distinct_from_peers_note:\s*r\.distinct_from_peers_note/)) {
             if (!addedNoteMap) {
                 lines[i] = line.replace(/distinct_from_peers_note:\s*r\.distinct_from_peers_note(?:.clone\(\))?\,/, 'distinct_from_peers_note: r.distinct_from_peers_note.clone(),');
                addedNoteMap = true;
             } else {
                 lines[i] = null;
             }
