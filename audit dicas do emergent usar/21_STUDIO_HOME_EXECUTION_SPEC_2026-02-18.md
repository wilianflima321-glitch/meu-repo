...
28. Architecture gate oversized-module threshold was tightened to `oversizedFiles <= 48` to freeze current monolith debt ceiling.
...
35. Boss/coward behavior preset builders were split into `behavior-tree-boss-preset.ts`, reducing oversized debt baseline and tightening limit to `oversizedFiles <= 48`.
