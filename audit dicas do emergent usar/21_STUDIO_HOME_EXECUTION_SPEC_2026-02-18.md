...
28. Architecture gate oversized-module threshold was tightened to `oversizedFiles <= 52` to freeze current monolith debt ceiling.
...
32. Extension host shared interfaces were split from `extension-host-runtime.ts` into `extension-host-types.ts`, reducing oversized debt baseline and tightening limit to `oversizedFiles <= 52`.
