...
- `LIMITATIONS.md` - Limitações técnicas detalhadas
- `COMPETITIVE_GAP.md` - Análise competitiva completa


---

## 8. EXECUTION DELTA (2026-02-13) - RESOLVED DUPLICATIONS

### 8.1 UI component duplication
- **Command Palette**
  - Canonical runtime provider: `cloud-web-app/web/components/ide/CommandPalette.tsx`
  - Duplicate imports blocked by QA: `cloud-web-app/web/scripts/check-canonical-components.mjs`
- **Status/toolbar visual language**
  - Workbench controls standardized to codicon-based wrapper: `cloud-web-app/web/components/ide/Codicon.tsx`

### 8.2 File API authority duplication
- Canonical file authority remains:
  - `/api/files/tree`
  - `/api/files/fs`
- Deprecated workspace file routes now return explicit deprecation (`410`):
  - `/api/workspace/tree`
  - `/api/workspace/files`

### 8.3 Auth model consistency
- Session endpoints are explicitly decommissioned under JWT-only policy:
  - `/api/auth/sessions`
  - `/api/auth/sessions/[id]`
- Response contract now returns explicit `DEPRECATED_ROUTE` instead of ambiguous session state.

### 8.4 Route inventory reliability
- Route inventory now generated by script (no hardcoded stale counters):
  - `cloud-web-app/web/scripts/generate-routes-inventory.mjs`
  - Output: `cloud-web-app/web/docs/ROUTES_INVENTORY.md`
